...
Процедура СтатусыДокументовИнициализация(context_params = Неопределено) Экспорт
	...
	Если СбисСтатусыДокументов = Неопределено Тогда
		СбисСтатусыДокументов = Новый Структура;
		СбисСтатусыДокументов.Вставить("ИмяРегистраСвойствОбъектов", СтатусыДокументовИмяРегистраСвойствОбъектов());
		СбисСтатусыДокументов.Вставить("ИмяПланаВидовХарактеристикСвойствОбъектов",
			СтатусыДокументовИмяПланаВидовХарактеристикСвойствОбъектов());
		СбисСтатусыДокументов.Вставить("СправочникСвойствОбъектов", СтатусыДокументовИмяСправочникаСвойствОбъектов());
		...
		СбисСтатусыДокументов.Вставить("СвойстваВРазрезеКодаСервиса", СвойстваВРазрезеКодаСервиса);
		
		СбисСтатусыДокументов.Вставить("КэшСвойствОбъектов", Новый Соответствие);
		
		СбисСтатусыДокументов.Вставить("Сервис", "---undefine---"); // Обновится при 1-ом запуске
		СбисСтатусыДокументов.Вставить("Аккаунт", "");
		
		context_params.Вставить("ИмяРегистраСтатусов", СбисСтатусыДокументов.ИмяРегистраСвойствОбъектов);
	...
	СбисСтатусыДокументов.Свойство("Сервис", ТекущийСервис);
	Если Сервис <> ТекущийСервис Тогда
		СбисСтатусыДокументов.Вставить("Сервис", Сервис);
		// При первом запуске надо создать записи в ПВХ для хранения статусов и идентификаторов в регистрах сведений
		Если ЗначениеЗаполнено(СбисСтатусыДокументов.ИмяПланаВидовХарактеристикСвойствОбъектов) Тогда
			СтатусыДокументовДобавитьЗаписиВПланВидовХарактеристик();
		КонецЕсли;
		СбисСтатусыДокументов.Вставить("Аккаунт", get_prop(context_params, "account", "") );
	КонецЕсли;
...
Процедура СтатусыДокументовДобавитьЗаписиВПланВидовХарактеристик()
	ИмяПланаВидовХарактеристикСвойствОбъектов 	= СбисСтатусыДокументов.ИмяПланаВидовХарактеристикСвойствОбъектов;
	СправочникСвойствОбъектов 					= СбисСтатусыДокументов.СправочникСвойствОбъектов; 
	...
	КонецЕсли;  
	СтатусыДокументовДобавитьЗаписьВПланВидовХарактеристик(Новый Структура("Имя", СбисСтатусыДокументов.Сервис
		+ "ДокументСБИС_Ид"));
	СтатусыДокументовДобавитьЗаписьВПланВидовХарактеристик(Новый Структура("Имя", СбисСтатусыДокументов.Сервис
		+ "ДокументСБИС_Статус"));
	СтатусыДокументовДобавитьЗаписьВПланВидовХарактеристик(Новый Структура("Имя", СбисСтатусыДокументов.Сервис
...
Функция СтатусыДокументовПолучитьКлючСвойстваДокумента(ОписаниеСвойства)
	...
	Если Не СбисСтатусыДокументов.СвойстваВРазрезеКодаСервиса.НайтиПоЗначению(НРег(КлючСвойства))
			= Неопределено Тогда
		КлючСвойства = СбисСтатусыДокументов.Сервис + КлючСвойства;
	КонецЕсли;
...
Функция СтатусыДокументовПолучитьСвойствоОбъекта(ОписаниеСвойства)
	КлючСвойства = СтатусыДокументовПолучитьКлючСвойстваДокумента(ОписаниеСвойства);
	СсылкаНаСвойство = СбисСтатусыДокументов.КэшСвойствОбъектов.Получить(КлючСвойства);
...
Функция СтатусыДокументовИзвлечьСвойствоПоОписанию(КлючСвойства)
	СсылкаНаСвойство = ПланыВидовХарактеристик[СбисСтатусыДокументов.ИмяПланаВидовХарактеристикСвойствОбъектов]
							.НайтиПоНаименованию(КлючСвойства, Истина).Ссылка;
	СбисСтатусыДокументов.КэшСвойствОбъектов.Вставить(КлючСвойства, СсылкаНаСвойство);
	Возврат СсылкаНаСвойство;
КонецФункции
...
Функция СтатусыДокументовПолучитьСостоянияОбъектов(tUID) Экспорт
	...
	КонецЕсли;
	Сервис  = СбисСтатусыДокументов.Сервис;
	Аккаунт = СбисСтатусыДокументов.Аккаунт;
	
	Если СбисСтатусыДокументов.ИмяРегистраСвойствОбъектов = "Saby_Состояние" Тогда
...
Процедура СтатусыДокументовОбновитьПоUID(UID, АктивныйЭтап = "", Состояние = Неопределено,
	...
	Если СбисСтатусыДокументов = "Saby_Состояние" Тогда
...
Функция СтатусыДокументовПоДопСведениямОтобратьПоСпискуОбъектов(мОбъекты, Сервис = "", Аккаунт = "") Экспорт
	...
	ИмяПВХ = СбисСтатусыДокументов.ИмяПланаВидовХарактеристикСвойствОбъектов;
	...
	Запрос.Текст = "
        ...
        |ИЗ
        |	РегистрСведений." + СбисСтатусыДокументов.ИмяРегистраСвойствОбъектов + " КАК ЗначенияСвойствОбъектов
        |ГДЕ
...
Функция СтатусыДокументовПоДопСведениямУдалитьСвойствоОбъекта(ИмяСвойства, ДокументСсылка) Экспорт
	...
	ИмяРегистраСвойствОбъектов = СбисСтатусыДокументов.ИмяРегистраСвойствОбъектов;
