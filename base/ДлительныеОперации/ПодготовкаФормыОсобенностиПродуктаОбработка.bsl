...
Процедура ЗаполнитьПутьОбработкиДляВыполненияФоновогоЗадания()
	...
	Если Найти(ИмяФайлаОбработки, ".epf") = 0 Тогда 
		ЭтаФорма.ИмяОбработки = ПолучитьДополнительнуюОбработку();			
	Иначе	
		...
		Если ФайлСуществует Тогда   
			...
			ЭтаФорма.ИмяОбработки = КопияОбработкиНаСервере(Адрес);
		Иначе
			ЭтаФорма.ИмяОбработки = КопияОбработкиНаСервере(ИмяФайлаОбработки);	
		КонецЕсли;
	КонецЕсли;	
КонецПроцедуры
