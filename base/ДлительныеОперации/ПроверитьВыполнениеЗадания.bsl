...
&НаКлиенте
Процедура ОбработатьПрочиеСтатусы(Параметры)
...
Процедура Подключаемый_ОбработатьЗадание(Задание)
	...
	Если Задание.Прогресс <> Неопределено Тогда
		...
		Если Не ПустаяСтрока(ПрогрессСтрокой) Тогда
			...
			ЭтаФорма.ТекстОповещения = Задание.Прогресс.Текст;
		КонецЕсли;
	КонецЕсли;
...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	ИначеЕсли Статус = "Ошибка" Тогда
		...
		Возврат;
	Иначе
		ОбработатьПрочиеСтатусы(Параметры);	
	КонецЕсли;
...
Процедура ПоказатьОповещение()
	...
	Если ПояснениеОповещения = Неопределено И ВладелецФормы <> Неопределено
		И ВладелецФормы.Окно <> Неопределено Тогда
		ПояснениеОповещения = ВладелецФормы.Окно.Заголовок;
	КонецЕсли;
	
	Если 	Параметры.ФоновоеЗаданиеНаименование = "Загрузка в " + ЛокализацияНазваниеПродукта()
		...
		НавигационнаяСсылкаОповещения = АдресСтраницыОтчетОЗагрузке(ПараметрыЗагрузки["params"]["api_url"], 
	...
	Пояснение = ?(Оповещение.Текст <> Неопределено, Оповещение.Текст, НСтр("ru = 'Действие выполнено'"));
	ПоказатьОповещениеПользователя(ПояснениеОповещения,НавигационнаяСсылкаОповещения, 
		?(НЕ ПустаяСтрока(ТекстОповещения),ТекстОповещения, Пояснение)
		,КартинкаОповещения
...
&НаСервере
Функция ПроверитьЗаданиеВыполнено(Знач ФормаЗакрывается)
