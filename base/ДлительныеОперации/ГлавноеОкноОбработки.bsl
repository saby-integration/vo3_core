...
Процедура ПоказатьОповещение()
	...
	Если 	_Параметры.ФоновоеЗаданиеНаименование = "Загрузка в "+ЛокализацияНазваниеПродукта() //команда - "Загрузка в СБИС"
				или 
		...
		НавигационнаяСсылкаОповещения = АдресСтраницыОтчетОЗагрузке(ПараметрыЗагрузки["params"]["api_url"],ПараметрыЗагрузки["operation_uuid"]);
	...
	ПоказатьОповещениеПользователя(ПояснениеОповещения,НавигационнаяСсылкаОповещения, 
		?(НЕ ПустаяСтрока(ПараметрыСеансаРасширения.ТекстОповещения),ПараметрыСеансаРасширения.ТекстОповещения, Пояснение)
