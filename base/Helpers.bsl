...
Функция ПолучитьСсылкуПоИдИС(Знач ИмяИС, ИдИС) Экспорт
	Попытка
		мИмяИС = Saby_СтрРазделить82(ИмяИС,".");
...
Функция ПреобразоватьВерсию(ВерсияМетаданных) Экспорт
	...
	Версия = СтрЗаменить(Версия,"\","");
	МассивПодстрок = Saby_СтрРазделить82(Версия,".");
	Версия = "";
...
Функция ПроверкаПринадлежностиЭлементу(Элемент1, Элемент2)
	Попытка
		Возврат Элемент1.ПринадлежитЭлементу(Элемент2);
	Исключение
		Возврат Ложь;
	КонецПопытки;	
КонецФункции
...
Функция fill_action(action, status, subtitle)
	...
	ДобавитьТриЧаса = КоличествоЧасов * КоличествоСекундВЧасе;
	ФорматДаты = "ДФ='yyyy-MM-dd HH:mm:ss'";
	action.begin = Формат(Дата(1, 1, 1) + action.begin / 1000 + ДобавитьТриЧаса, ФорматДаты) + "." + (action.begin%1000);
	action.end = Формат(Дата(1, 1, 1) + action.end / 1000 + ДобавитьТриЧаса, ФорматДаты) + "." + (action.end%1000);
...
Функция РазобратьСтрокуУРЛ(Знач СтрокаАдреса, ПротоколПоУмолчанию = "http")
	...
	Если Индекс > 0 Тогда
		...
		Адрес.port = ТипЧисло.ПривестиЗначение(Адрес.port);
