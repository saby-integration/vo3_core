...
Процедура ОбработатьПолучение(НазваниеСбисСобытия,КлючСбисСобытия)
...
Процедура ОбработатьУведомление(НазваниеСбисСобытия,КлючСбисСобытия)
...
Процедура ОбработатьИзвещение(НазваниеСбисСобытия,КлючСбисСобытия)
...
Процедура ОбработатьПодтверждение(НазваниеСбисСобытия,КлючСбисСобытия)
...
Процедура ОбработатьСоставПакета(Событие, СбисСобытия) 
	КлючСбисСобытия		= "";
	ЗаполнитьПодпись	= Истина;
	НазваниеСбисСобытия	= Событие["Название"]; 
	Если		Лев(НазваниеСбисСобытия, 9)  = "Получение" Тогда
		ОбработатьПолучение(НазваниеСбисСобытия,КлючСбисСобытия);	
	ИначеЕсли	Лев(НазваниеСбисСобытия, 11) = "Уведомление" Тогда
		ОбработатьУведомление(НазваниеСбисСобытия,КлючСбисСобытия);	
	ИначеЕсли	Лев(НазваниеСбисСобытия,9)	= "Извещение" Тогда
		ОбработатьИзвещение(НазваниеСбисСобытия,КлючСбисСобытия);	
	ИначеЕсли	Лев(НазваниеСбисСобытия,13)	= "Подтверждение" Тогда
		Если Найти(НазваниеСбисСобытия, "получателю на извещение") Тогда 
			Возврат;
		КонецЕсли;	
		ЗаполнитьПодпись = Ложь;
		ОбработатьПодтверждение(НазваниеСбисСобытия,КлючСбисСобытия);
	Иначе
		КлючСбисСобытия		= "";	
	КонецЕсли;
...
// Формирует ленту о прохождении документа
//
// Параметры:
//  СоставПакета - Структура - состав пакета.
//  ДопПараметры - Произвольный - параметры.
//
Процедура ЗаполнитьПрохождение(СоставПакета, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого Событие Из СоставПакета["Событие"] Цикл
		...
		Если ЭтоСистемноеСобытие Тогда
			Продолжить;
		КонецЕсли;
		ОбработатьСоставПакета(Событие, СбисСобытия);
	КонецЦикла;
...
Функция ПолучитьПредставлениеСертификата(Сертификат, Шаблон, СокрФИО = Ложь, ФорматДаты = "ДФ=dd.MM.yyyy", Разделитель = ".") Экспорт
	...
	МассивЧастейШаблона = Saby_СтрРазделить82(Шаблон, Разделитель);
	Результат = "";
