...
Функция ИдентификаторМодуляКомпонентыИнтеграции(ИмяМодуля)
	Если Не ЗначениеЗаполнено(МодульКомпонентыИнтеграции) Тогда 
		...
		Исключение   
			ВызватьИсключение "Отсутствует модуль " + Строка(ИмяМодуля);
		КонецПопытки;	
	КонецЕсли;
...
Функция AuthConfirmCode(context_params, Код, ResourceId, ВызываемыйМетод) Экспорт 
	MethodName = "ExtSdk2.AuthConfirmCode";
	Хост = СтрЗаменить(get_prop(context_params, "ApiUrl",""), "https://", "");
	Хост = СтрЗаменить(Хост, "ie-1c", "online"); 
	ДопПараметры = Новый Структура("БезАвторизации,Хост,ОчиститьПараметры", Истина, Хост, Истина);
	Param = Новый Структура("Код,Идентификатор,ВызываемыйМетод", Код, ResourceId, ВызываемыйМетод);
	Call_param = Новый Структура("Param", Param);
	res = CallMethod(MethodName, Call_param, context_params, , ДопПараметры);
	res = get_prop(res, "result");
	Возврат get_prop(res, "Result"); 
КонецФункции
...
Функция DetailsПоОшибкеExtSdk2(Ошибка)
	details = get_prop(Ошибка, "details");
	Если get_prop(Ошибка, "code") = 303000 Тогда
		details = Новый Соответствие;
		details.Вставить("code", 303);
		dump = get_prop(Ошибка, "dump");
		Если dump <> Неопределено  Тогда  
			details.Вставить("addinfo", dump);
			Возврат details
		КонецЕсли;	
		Для Каждого Запись Из Ошибка.stack Цикл 
			dump = get_prop(Запись, "dump");
			Если dump = Неопределено Тогда
				Продолжить;
			КонецЕсли;
