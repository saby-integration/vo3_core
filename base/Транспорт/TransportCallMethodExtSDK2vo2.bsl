...
//
//DynamicDirective
Функция CallMethod(Знач MethodName, ПараметрыВызова, context_params)
	Result = Неопределено;
	...
	Отказ = Ложь;
	Результат = ТранспортИнтеграции.ВладелецФормы.Кэш.Интеграция.СбисОтправитьИОбработатьКоманду(ТранспортИнтеграции.ВладелецФормы.Кэш, MethodName, ПараметрыВызова, ПараметрыКоманды, Отказ);
