...
Процедура SabyHttpsClient_check_answer_requests() 
	...
	Для Каждого Запись Из async_requests Цикл
		Deadline = get_prop(Запись.Значение, "Deadline");
		Если ТипЗнч(Deadline) <> Тип("Дата") Тогда
			Продолжить;
		КонецЕсли;	
		// BSLLS:DeprecatedCurrentDate-off Использование ТекущаяДата() на Клиенте
		Если Deadline < ТекущаяДата() Тогда 
			ИдентификаторыДляУдаления.Добавить(Запись.Ключ);
			ПараметрыЗапроса = async_requests.Получить(Запись.Ключ);  
			РезультатЗапроса = Новый Структура;
			РезультатЗапроса.Вставить("code", 403);
	        Error = Новый Структура;
			Error.Вставить("message", "Ошибка работы с Интернет:  Превышен таймаут");
			Error.Вставить("detail", "За отведенное время не удалось получить ответ от компоненты SabyHttpsClient"); 
			result = Новый Структура("Error", Error);
			РезультатЗапроса.Вставить("result", result); 
			async_responce = Новый Структура("ПараметрыЗапроса,РезультатЗапроса", ПараметрыЗапроса, РезультатЗапроса);
			async_responces.Вставить(Запись["QueryId"], async_responce);   	
		КонецЕсли;
		// BSLLS:DeprecatedCurrentDate-on	
	КонецЦикла;	 
	Для Каждого Запись Из ИдентификаторыДляУдаления Цикл
		async_requests.Удалить(Запись);	
	КонецЦикла;	
КонецПроцедуры
...
Функция SabyHttpsClient_get_answer_request(QueryId = Неопределено) 
	...
	Для Каждого Запись ИЗ Результат Цикл 
		...
		КонецЕсли;	
	КонецЦикла;	
	SabyHttpsClient_check_answer_requests();
...
#Область include_core_base_Транспорт_SabyHttpsClient_ВыполнятьНаСервере
#КонецОбласти	
...
Функция ОбработкаРезультатаЗапросаSabyHttpsClient(РезультатЗапроса)  
	...
	Если code <> 200 И code <> 201 Тогда  
		res = Новый Структура("Error", get_prop(Результат, "error")); 
