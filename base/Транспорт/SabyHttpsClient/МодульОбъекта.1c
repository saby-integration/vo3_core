...
Функция ТранспортSabyHttpsClient(context_params) Экспорт
	Если ТипЗнч(context_params) <> Тип("Структура") Тогда
		Возврат Ложь;
	КонецЕсли;
	Если НЕ context_params.Свойство("public") Тогда
		Возврат Ложь;
	КонецЕсли;
	Если НЕ context_params.public.Свойство("exchange_method") Тогда
		Возврат Ложь;
	КонецЕсли;
	Если context_params.public.exchange_method = "SabyHttpsClient" Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Функция local_helper_exec_request_SabyHttpsClient(request_type, Знач request_url, response_type, ПараметрыВыполнения, context_params = Неопределено)	
	...
	Если ПараметрыВыполнения <> Неопределено
		И ПараметрыВыполнения.Свойство("multithread_mode") 
		И ПараметрыВыполнения["multithread_mode"] = Истина Тогда 
		Deadline = ТекущаяДатаСеанса() + get_prop(ПараметрыВыполнения,"timeout",60);
