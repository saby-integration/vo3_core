...
Функция local_helper_exec_request_HTTPСоединение(request_type, Знач request_url, response_type,
	...
	Если Proxy = Неопределено и ProxyParam <> Неопределено Тогда
		Попытка
			ИспользоватьАутентификациюОС = get_prop(ProxyParam, "ProxyUseOSAuthentication", Ложь);
		...
		Исключение
			...
			ВызватьИсключение NewExtExceptionСтрока(ИнфОбОшибке, "Ошибка установки параметров прокси-сервера.");
		КонецПопытки;
	КонецЕсли;
	
	headers = get_prop(kwargs, "headers", Новый Соответствие);
	default_timeout = 180;
	timeout = get_prop(context_params, "Timeout", default_timeout);
	Попытка
		http_connection = Новый HTTPСоединение(Адрес.host, Адрес.port, , , Proxy, timeout, ssl, ИспользоватьАутентификациюОС);
