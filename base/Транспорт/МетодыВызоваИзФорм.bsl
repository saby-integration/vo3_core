...
Процедура ВойтиТранспорт(context_params, Кэш = Неопределено) Экспорт
	...
	ИначеЕсли ВидТранспорта = "SabyConnect" Тогда	
		ВойтиКаталог(context_params);	
	Иначе
...
Процедура ВойтиКаталог(context_params) 
	СтруктураСессии = Новый Структура;
	СтруктураСессии.Вставить("Session", "Ok");
	ЗаписатьСессию(context_params, СтруктураСессии);
	НастройкиПодключенияЗаписать(context_params);
КонецПроцедуры
...
Процедура ОчиститьКомпонентуИнтеграции() Экспорт
	// BSLLS:UnusedLocalVariable-off Переменная используется в склееных областях
	КомпонентаИнтеграции = Неопределено; 
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда
		Для Каждого ЭлементАккаунт Из Результат Цикл
			...
			Если Не (НазваниеАккаунта = "Физики" Или НазваниеАккаунта = "Личный аккаунт") Тогда
				НомерАккаунта = get_prop(ЭлементАккаунт, "НомерАккаунта");
				СписокАккаунтов.Добавить(НомерАккаунта,	НазваниеАккаунта + " (" + НомерАккаунта + ")"); 
			КонецЕсли;		
		КонецЦикла;
	КонецЕсли;
