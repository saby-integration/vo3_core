...
Процедура ОткрытьВСБИС_ПолучитьURL( Источник, ПараметрКоманды ) Экспорт 
	...
	Исключение
		...
		Если СтруктураОшибки.type = "Unauthorized" Тогда
			...
			ВходящиеПараметры	= Новый Структура("Источник, ПараметрКоманды",Источник,ПараметрКоманды);
			ПроверкаВведенныхДанныхАутентификации = Новый ОписаниеОповещения("ОткрытьВСБИС_ПолучитьURL_ПослеАутентификации", ЭтотМодуль(), ВходящиеПараметры);
