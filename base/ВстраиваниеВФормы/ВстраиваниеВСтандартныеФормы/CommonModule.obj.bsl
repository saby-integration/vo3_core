...
Функция ИмяТипаОбъекта(Контекст) Экспорт
...
Процедура ЗаполнитьСтруктуруПолейДляКолонкиСтатусФизЛица(Контекст, НаименованиеРеквизитаСписок, ПарамтерыДопТаблицы)
	ПоляСоединения = Новый Соответствие;
	ПоляСоединения.Вставить(ПарамтерыДопТаблицы.ИмяКолонкиЛевогоСоединения, "ФизЛицо");
	ПарамтерыДопТаблицы.Вставить("ПоляСоединения", ПоляСоединения);

	МассивКолонок = Новый Массив;
	КолонкаДопТаблицы = Новый Структура;
	КолонкаДопТаблицы = Новый Структура;
	КолонкаДопТаблицы.Вставить("Имя",				"СтатусФизЛица");
	КолонкаДопТаблицы.Вставить("ПутьКДанным",		"ЕСТЬNULL(" + ПарамтерыДопТаблицы.Псевдоним + ".СтатусФизЛица, """")");
	КолонкаДопТаблицы.Вставить("Заголовок",			"Статус кандидата");
...
Процедура ЗаполнитьСтруктуруПолейДляКолонкиКЭДО(Контекст, НаименованиеРеквизитаСписок, ПарамтерыДопТаблицы)
	...
	МассивКолонок = Новый Массив;
	КолонкаДопТаблицы = Новый Структура;
	КолонкаДопТаблицы.Вставить("Имя",        "КартинкаКЭДО");
	...
	КолонкаДопТаблицы = Новый Структура;
	КолонкаДопТаблицы.Вставить("Имя",        "УчастникКЭДО");
	КолонкаДопТаблицы.Вставить("ПутьКДанным",    "Выразить(ЕСТЬNULL(" + ПарамтерыДопТаблицы.Псевдоним + ".КЭДО, Ложь) КАК БУЛЕВО)" );
	КолонкаДопТаблицы.Вставить("Заголовок",      "КЭДО");
	КолонкаДопТаблицы.Вставить("Картинка",      Неопределено);
...
// BSLLS:QueryParseError-on	
	...
	УспешноеВыполнение = ИзменитьЗапросДинамическогоСписока(Контекст.Форма, ПараметрыДопТаблицы);
...
Процедура ЗаполнитьСтруктуруПолейДляКолонокСостоянияОбъектов(Контекст, НаименованиеРеквизитаСписок, ПарамтерыДопТаблицы)
	...
	КолонкаДопТаблицы  = Новый Структура;
	КолонкаДопТаблицы.Вставить("Имя",        "СтатусВ_"+НазваниеПродукта);
	КолонкаДопТаблицы.Вставить("ПутьКДанным",    "ЕСТЬNULL("+ПарамтерыДопТаблицы.Псевдоним+".Состояние, НЕОПРЕДЕЛЕНО)" );
	КолонкаДопТаблицы.Вставить("Заголовок",      "Статус в "+НазваниеПродукта);
	КолонкаДопТаблицы.Вставить("ОтображатьНаФорме",  Ложь);
	МассивКолонок.Добавить(КолонкаДопТаблицы);
...
Процедура ДобавитьКолонкиСтатусаСостояния(Контекст, НаименованиеРеквизитаСписок, ИмяКолонкиЛевогоСоединения="Документ", context_params=Неопределено)
	...
	АккаунтИнтеграции = МодульCore.СвойствоСессии(context_params,"account");
	СвойствоАктивныеЭтап = СервисИнтеграции + "ДокументСБИС_ТекущийЭтап";
	СвойствоСостояние = СервисИнтеграции + "ДокументСБИС_Статус";
	
	ТекстЗапросаКЭДО = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                   |	ЕСТЬNULL(АктивныеЭтап.Объект, СтатусДокумента.Объект) КАК Объект,
	                   |	МАКСИМУМ(АктивныеЭтап.Значение) КАК АктивныйЭтап,
	                   |	МАКСИМУМ(Saby_СостоянияОбъектов.Ссылка) КАК Состояние
	                   ...
	                   |	ГДЕ
	                   |		ДополнительныеСведения.Свойство.Наименование = &СвойствоСостояние) КАК СтатусДокумента
	                   ...
	                   |			РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
	                   |		ГДЕ
	                   |			ДополнительныеСведения.Свойство.Наименование = &СвойствоАктивныеЭтап) КАК АктивныеЭтап
	                   |		ПО (АктивныеЭтап.Объект = СтатусДокумента.Объект)
					   |	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Saby_СостоянияОбъектов КАК Saby_СостоянияОбъектов ПО СтатусДокумента.Значение = Saby_СостоянияОбъектов.ИдентификаторСБИС 
					   |СГРУППИРОВАТЬ ПО 
					   |	ЕСТЬNULL(АктивныеЭтап.Объект, СтатусДокумента.Объект);
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ * ИЗ 
					   |";
	...
	Фильтр.Добавить(Новый Структура("Параметр, Значение", "СвойствоАктивныеЭтап", СвойствоАктивныеЭтап ));
...
Процедура ДобавитьКолонкиВДинамическийСписокПриИзмененииЗапроса(Форма, ИмяРеквизитаСписок) Экспорт
	...
	ИмяТипа = ИмяТипаОбъекта(СтруктураОписаниеФормы);
