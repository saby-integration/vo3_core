...
Процедура СозданиеЭлементовУправленияНазначениеДействий(Форма, Команды)
	...
	ГруппаИнтеграция.Заголовок = ЛокализацияНазваниеПродукта();
...
Процедура ДобавитьКомандыНаФорму(Форма, ПараметрыРазмещения, КомандыПустаяТаблица)
	...
	УсловиеДЛяОтказаВывода = 
		...
		Или НЕ МодульВстраиваниеВСтандартныеФормы.ЭтотОбъектМетаданныхЗахвачен(СтруктураОписаниеФормы);
...
Процедура ДобавитьКолонкиСтатусаКЕДО(Контекст, НаименованиеРеквизитаСписок,
	ИмяКолонкиЛевогоСоединения = "Сотрудник", context_params = Неопределено)
	ТекстЗапросаКЭДО = 
// BSLLS:QueryParseError-off
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ДобавитьКолонкиСтатусаСостояния(Контекст, НаименованиеРеквизитаСписок, ИмяКолонкиЛевогоСоединения="Документ", context_params=Неопределено)
	...
	Если АккаунтИнтеграции <> Неопределено Тогда
		УсловиеЗапроса = "
	|	ГДЕ
	...
	Фильтр	= Новый Массив();
	// Сервис
	Фильтр.Добавить(Новый Структура("Параметр, Значение", "Сервис", СервисИнтеграции ));
	// Аккаунт
	
	Фильтр.Добавить(Новый Структура("Параметр, Значение", "Аккаунт", АккаунтИнтеграции ));
...
Процедура ДобавитьКолонкиВДинамическийСписокПриИзмененииЗапроса(Форма, ИмяРеквизитаСписок) Экспорт
	...
	Если ТипОбъекта = "Справочник" Тогда
		...
		Если ИмяТипа = "ФизическиеЛица" Тогда
			ДобавитьКолонкиСтатусаФизЛица(СтруктураОписаниеФормы, НаименованиеРеквизитаСписок, "Ссылка", context_params);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьКолонкиВДинамическийСписок(Форма, ПараметрыРазмещения = Неопределено) Экспорт
	...
	КонецЕсли;
	Если ТипОбъекта = "Документ" Тогда
		Если ИмяТипа <> "ШтатноеРасписание" Тогда
			ДобавитьКолонкиСтатусаКЕДО(СтруктураОписаниеФормы, НаименованиеРеквизитаСписок, ПолеСтыковкиСотрудник, context_params);
		КонецЕсли;
		ДобавитьКолонкиСтатусаСостояния(СтруктураОписаниеФормы, НаименованиеРеквизитаСписок, "Ссылка", context_params);
	КонецЕсли;	
