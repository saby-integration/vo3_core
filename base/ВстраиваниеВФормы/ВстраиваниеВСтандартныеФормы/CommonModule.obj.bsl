...
Процедура ПолучитьСписокКоманд( Источники, Команды ) Экспорт 
	...
	ДобавитьКомандуНаФорму1С(НоваяКоманда, Команды, МассивТиповДокументов, ПорядокСчет);
...
Процедура ОбходОшибкиКонтрольВеденияУчетаПриСозданииНаСервереФормыСписка(ДанныеФормы, ПараметрыДопТаблицы, мИменаВременныхТаблиц)
// BSLLS:DuplicateStringLiteral-off
// Используется разный регист

	// КОСТЫЛЬ
	// Пишем строчными. обходим алгоритмы 1с. поистка подстроки которые рушат весь запрос
	// КонтрольВеденияУчета Процедура ПриСозданииНаСервереФормыСписка
	ПолныйТекстЗапроса = ДанныеФормы.Форма[ПараметрыДопТаблицы.ИмяСпискаНаФорме].ТекстЗапроса;
	Для Каждого ИмяВрТаблицы Из мИменаВременныхТаблиц Цикл
		 ПолныйТекстЗапроса =  СтрЗаменить(ПолныйТекстЗапроса, "ПОМЕСТИТЬ " + ИмяВрТаблицы, "Поместить " + ИмяВрТаблицы);
	КонецЦикла;
	ПолныйТекстЗапроса =  СтрЗаменить(ПолныйТекстЗапроса, "ВЫБРАТЬ", "Выбрать");
	ПолныйТекстЗапроса =  СтрЗаменить(ПолныйТекстЗапроса, "ПОМЕСТИТЬ", "Поместить");
	// Находим послеюднюю ВЫБРАТЬ и пишем её ЗАГЛАВНЫМИ символами
	
	мСтрокиЗапроса = СтрРазделить82(ПолныйТекстЗапроса, Символы.ПС);
	СчетСтрок = -1;
	Пока СчетСтрок < мСтрокиЗапроса.Количество() - 1 Цикл
		СчетСтрок = СчетСтрок + 1;
		СтрокаЗапроса = мСтрокиЗапроса[СчетСтрок];
		Если СтрНачинаетсяС(СокрЛП(СтрокаЗапроса), "Выбрать") Тогда
			СтрокаЗапроса =  СтрЗаменить(СтрокаЗапроса, "Выбрать", "ВЫБРАТЬ");
			мСтрокиЗапроса[СчетСтрок] = СтрокаЗапроса;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	СчетСтрок = мСтрокиЗапроса.Количество();
	ПризнакВыбрать = Ложь;
	ПризнакПоместить = Ложь;
	Пока СчетСтрок > 1 Цикл
		СчетСтрок = СчетСтрок - 1;
		СтрокаЗапроса = мСтрокиЗапроса[СчетСтрок];
		Если НЕ ПризнакВыбрать И СтрНачинаетсяС(СокрЛП(СтрокаЗапроса), "Выбрать") Тогда
			СтрокаЗапроса =  СтрЗаменить(СтрокаЗапроса, "Выбрать", "ВЫБРАТЬ");
			мСтрокиЗапроса[СчетСтрок] = СтрокаЗапроса;
			ПризнакВыбрать = Истина;
		КонецЕсли;
		Если НЕ ПризнакПоместить И СтрНачинаетсяС(СокрЛП(СтрокаЗапроса), "Поместить") Тогда
			СтрокаЗапроса =  СтрЗаменить(СтрокаЗапроса, "Поместить", "ПОМЕСТИТЬ");
			мСтрокиЗапроса[СчетСтрок] = СтрокаЗапроса;
			ПризнакПоместить = Истина;
		КонецЕсли;
		Если ПризнакВыбрать И ПризнакПоместить Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	ПолныйТекстЗапроса = СтрСоединить82(мСтрокиЗапроса, Символы.ПС);
	ДанныеФормы.Форма[ПараметрыДопТаблицы.ИмяСпискаНаФорме].ТекстЗапроса = ПолныйТекстЗапроса;
...
// BSLLS:QueryParseError-on
	...
	Если УспешноеВыполнение Тогда
		...
		ОбходОшибкиКонтрольВеденияУчетаПриСозданииНаСервереФормыСписка(ДанныеФормы, ПрмДопТаблицы, мИменаВременныхТаблиц);
	КонецЕсли;
...
// BSLLS:QueryParseError-on
	...
	Если УспешноеВыполнение Тогда
		УспешноеВыполнение = ДобавитьКолонкиНаФорму(Контекст.Форма, ПараметрыДопТаблицы);
		ОбходОшибкиКонтрольВеденияУчетаПриСозданииНаСервереФормыСписка(Контекст, ПараметрыДопТаблицы, мИменаВременныхТаблиц);
	КонецЕсли;
