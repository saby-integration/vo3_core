...
Процедура ПолучитьСписокКоманд( Источники, Команды ) Экспорт 
	...
	ЕстьКомандыДляСпрНоменклатура 	= Ложь;
	...
	Для Каждого ТипЗначения Из Источники Цикл
		...
		Если ЕстьКомандыДляСправочников Тогда
			...
			ЕстьКомандыДляСпрФизЛица = ЕстьКомандыДляСпрФизЛица Или Найти(ОбъектМетаданных.Имя, "ФизическиеЛица") > 0;
			ЕстьКомандыДляСпрНоменклатура 	= ЕстьКомандыДляСпрНоменклатура Или Найти(ОбъектМетаданных.Имя, "Номенклатура") > 0;
		КонецЕсли;
	...
	Если Не ЕстьКомандыДляСпрНоменклатура Тогда
		// Скрыть команду для справочника - номенклатура
		ПорядокСчет = ПорядокСчет + 1;
		НоваяКоманда = Новый Структура;
...
// Собираем запрос частями
	...
	МассивРезультат.Добавить(
               ...
               |	Объект
               |");
	Результат = Saby_СтрСоединить82(МассивРезультат, Символы.ПС);
...
// BSLLS:QueryParseError-on
	...
	мСтрокиЗапроса.Добавить(ЧастьТекстаЗапроса);
	ТекстЗапросаКЭДО = Saby_СтрСоединить82(мСтрокиЗапроса, Символы.ПС);
