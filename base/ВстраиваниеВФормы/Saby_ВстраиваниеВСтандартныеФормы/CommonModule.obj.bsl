...
Процедура ИзменитьЗапросДинамическогоСписка(Форма, ОтборыСписка, ДляВыбораСотрудников, ИмяРеквизитаСписок) Экспорт
	...
	КонецЕсли;
	//А затем наш код	
	Saby_ВстраиваниеВСтандартныеФормы.ДобавитьКолонкиВДинамическийСписокПриИзмененииЗапроса(Форма, ИмяРеквизитаСписок);
КонецПроцедуры

Процедура ЗаполнитьТипыОбъектовОповещения(Форма)
	//Для конфигурация без полного использования функций БСП
	Если Saby_Core.get_prop(Форма, "ТипыОбъектовОповещения") = Неопределено Тогда
...
Процедура ПолучитьСписокКомандSaby( Источники, Команды ) Экспорт 
	...
	Если ЕстьКомандыДляСпрСотрудники Тогда
		Порядок = Порядок + 1;	
				НоваяКоманда = Новый Структура;
		НоваяКоманда.Вставить("Имя",       "Saby_УчаствуетВКЭДО");
		НоваяКоманда.Вставить("Заголовок", "Установить/Снять признак КЭДО"); 
		НоваяКоманда.Вставить("Действие",  "Saby_КомандыОбменаДляФормыКлиент.УстановитьПризнакУчаствуетВКЭДО");
		НоваяКоманда.Вставить("Отключена", Ложь);
		НоваяКоманда.Вставить("Порядок",   Неопределено);
		НоваяКоманда.Вставить("ИмяИни",    Неопределено);
...
Процедура ДобавитьКомандуНаФорму1С(КомандаSaby, Команды, МассивТиповДокументов, Порядок)
	...
	ПодключаемаяКоманда.Назначение               = ЛокализацияНазваниеПродукта();
...
Функция ПроисходилЛиОбменДокументовСоСБИС( МассивСсылокНаДокументы, ДопПараметры=Неопределено) Экспорт 
	...
	Результат = ОбработкаСБИС.ПроисходилЛиОбменДокументовСоСБИС(МассивСсылокНаДокументы, ДопПараметры);	
	Возврат Результат; 
...
Функция ПолучитьССылкиНаДокументыВСБИС(МассивИсточникДанных, ПолучитьТикет = Ложь ) Экспорт
	...
	Результат = Обработка.ПолучитьССылкиНаДокументыВСБИС(МассивИсточникДанных, ПолучитьТикет);
...
Функция ПолучитьДействияДляОбъекта(СсылкаНаОбъект, context_params) Экспорт
	...
	ОбработкаСБИС = Обработки.SABY.Создать();
	Результат = ОбработкаСБИС.ПолучитьДействияДляОбъекта(СсылкаНаОбъект, context_params);
...
Функция ПолучитьСписокРегламентов(ВидОбъекта) Экспорт 
	МодульОбъекта = Обработки.SABY.Создать();
...
Процедура ЗаполнитьСтруктуруПолейДляКолонокСостоянияОбъектов(Контекст, НаименованиеРеквизитаСписок, ПарамтерыДопТаблицы)
	...
	КолонкаДопТаблицы.Вставить("Заголовок",			ЛокализацияНазваниеПродукта());
	...
	КолонкаДопТаблицы.Вставить("Заголовок",			"Состояние "+ЛокализацияНазваниеПродукта());
...
Процедура ДобавитьКолонкиСтатусаСостояния(Контекст, НаименованиеРеквизитаСписок, ИмяКолонкиЛевогоСоединения="Документ", context_params=Неопределено)
	...
	|ПОМЕСТИТЬ Saby_States
	|ИЗ
	|	(ВЫБРАТЬ
	|		Saby_Левая.Объект КАК Объект,
	|		МАКСИМУМ(Saby_Левая.Сервис) КАК Сервис,
	|		МАКСИМУМ(Saby_Левая.Аккаунт) КАК Аккаунт
	|	ИЗ
	|		РегистрСведений.Saby_Состояние КАК Saby_Левая
	|	ГДЕ
	|		(Saby_Левая.Сервис = &Сервис
	|					И Saby_Левая.Аккаунт = &Аккаунт
	|				ИЛИ Saby_Левая.Сервис = """"
	|					И Saby_Левая.Аккаунт = ""---undefine---""
	|				ИЛИ Saby_Левая.Сервис = """"
	|					И Saby_Левая.Аккаунт = """")
	|	
	|	СГРУППИРОВАТЬ ПО
	|		Saby_Левая.Объект) КАК ЛеваяОтбор
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Saby_Состояние КАК ПраваяДанные
	...
	ПарамтерыДопТаблицы = ИнициализироватьСтруктуруДопТаблицы();
	ПарамтерыДопТаблицы.Вставить("ТаблицаДанных",		"Saby_States");
	ПарамтерыДопТаблицы.Вставить("Псевдоним",			"Saby_States");
	ПарамтерыДопТаблицы.Вставить("ИмяСпискаНаФорме",	НаименованиеРеквизитаСписок);
	ПарамтерыДопТаблицы.Вставить("ВременнаяТаблица",	ТекстЗапросаКЭДО);
	ПарамтерыДопТаблицы.Вставить("ИмяКолонкиЛевогоСоединения",	ИмяКолонкиЛевогоСоединения);
	...
	Фильтр.Добавить(Новый Структура("Параметр, Значение", "Сервис", Saby_Core.get_prop(context_params,"service","") ));
	//Аккаунт
	Фильтр.Добавить(Новый Структура("Параметр, Значение", "Аккаунт", Saby_Core.get_prop(context_params,"account","") ));
...
#Область include_core_base_ВстраиваниеВФормы_SabyВстраиваниеВСтандартныеФормы_ДобавитьКолонкиВСправочники
#КонецОбласти
...
Функция ИзменитьЗапросДинамическогоСписока(Контекст, СтруктураПараметров)
	...
	Фильтр = Saby_Core.get_prop(СтруктураПараметров, "Фильтр", Новый Массив);
