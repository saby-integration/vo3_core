...
Функция ПодготовитьСтруктуруОписанияФормы(Форма, ПараметрыРазмещения = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			...
			ИначеЕсли 	Форма.ИмяФормы = "Обработка.ЖурналДокументовАвансовыеОтчеты.Форма.СписокДокументов"
...
Функция ПолучитьИзОбъекта1СОбъектИнтеграции(ВидОбъекта)
	...
	ИначеЕсли ВидОбъекта = "НевыходыВОрганизациях" Тогда
		...
		СписокОбъектов.Добавить("Allowance");
		СписокОбъектов.Добавить("Отпуск");
	ИначеЕсли ВидОбъекта = "СписаниеБезналичныхДенежныхСредств" Тогда
		СписокОбъектов.Добавить("ИсходящийПлатеж");
	ИначеЕсли ВидОбъекта = "ПоступлениеБезналичныхДенежныхСредств" Тогда
		СписокОбъектов.Добавить("ВходящийПлатеж");
	ИначеЕсли ВидОбъекта = "НачислениеПоСреднему" Тогда
		СписокОбъектов.Добавить("Отпуск");
	ИначеЕсли ВидОбъекта = "РегистрацияНевыходов" Тогда
		СписокОбъектов.Добавить("Прогул");
		СписокОбъектов.Добавить("Отпуск");
	Иначе
