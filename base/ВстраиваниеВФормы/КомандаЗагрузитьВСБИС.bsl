...
Процедура ПроверкаПередаваемыхДокументов(ПроверкаОбмена, ВопросыПользователю)
	...
	//	ВопросКПользователю = "Один или несколько документов уже отправлены и являются черновиками в "
	//		+ ЛокализацияНазваниеПродукта() + ". Загрузить их повторно?";
	//	ВопросыПользователю.Добавить( Новый Структура("ТекстВопроса, КодСтатуса", ВопросКПользователю, СтатусЧерновик) );
	//КонецЕсли;
	//Если КоличествоСОшибками > 0 Тогда
	//	ВопросКПользователю = "Один или несколько документов уже отправлены в " + ЛокализацияНазваниеПродукта()
	//		+ ", но обмен прошел с ошибкой " + ЛокализацияНазваниеПродукта() + ". Загрузить их повторно?";
	//	ВопросыПользователю.Добавить( Новый Структура("ТекстВопроса, КодСтатуса", ВопросКПользователю, СтатусОшибка) );
	//КонецЕсли;
