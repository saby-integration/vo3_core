	...
	Если ПроверкаОбмена.БылОбмен.Количество() Тогда
		...
		Исключение                  
			...
			Если СтруктураОшибки.type = "Unauthorized" или НеобходимаНеобходимоВвестиКод Тогда
				ВходящиеПараметры	= Новый Структура("Команда, ПараметрКоманды", Команда, ПараметрКоманды);
				ПроверкаВведенныхДанныхАутентификации = Новый ОписаниеОповещения("_ПослеАутентификации", ЭтотМодуль(), ВходящиеПараметры);
...
Процедура ЗагрузитьВСБИСДействие( Команда, ВхПараметры ) Экспорт 
	...
	Если ПроверкаОбмена.ОбменСразу.Количество() Тогда
		 ...
		 ФормаЗагрузкаДокументов = ФормаОсновнойОбработки("ЗагрузкаДокументов", ПутьКФормамОбработки(), ПараметрыВФормуЗД); 
