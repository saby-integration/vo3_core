...
Процедура ЗагрузитьВСБИС( Команда, ПараметрКоманды ) Экспорт 
	...
	Если ПроверкаОбмена.БылОбмен.Количество() Тогда
		...
		Попытка
			ДействияОбъекта = BlocklyExecutor.ПолучитьДействияДляОбъекта(ПроверкаОбмена.БылОбмен[0].Ссылка, context_params);
		Исключение                  
	...
	Если ТипЗнч(ДействияОбъекта) = Тип("Структура") И get_prop(ДействияОбъекта, "message2user") <> Неопределено Тогда
