...
Процедура ОтправитьРасчетныйЛистокВСБИС(ПараметрыВызова, context_params) Экспорт	
	Saby_ФоновыеЗаданияКлиент.ЗапуститьINIФоновымЗаданием("РасчетныйЛисток_send", ПараметрыВызова, context_params, Неопределено, "Отправка в "+ЛокализацияНазваниеПродукта());	
КонецПроцедуры 

&НаКлиенте
Процедура ВыполнитьПроизвольныйИНИ(ИмяИНИФайла, ПараметрыВызова) Экспорт	
	Saby_ФоновыеЗаданияКлиент.ЗапуститьINIФоновымЗаданием(ИмяИНИФайла, ПараметрыВызова, Неопределено, Неопределено, "Отправка в "+ЛокализацияНазваниеПродукта());	
КонецПроцедуры 
...
Процедура ОтправитьСокращенныйРасчетныйЛистокВСБИС(ПараметрыВызова, context_params) Экспорт
	...
	Если  НЕ ПроверитьПолнотуДанныхОтбораДляВызоваИНИ(ПараметрыВызова) Тогда
		Возврат;
	КонецЕсли;
	Saby_ФоновыеЗаданияКлиент.ЗапуститьINIФоновымЗаданием("СокращенныйРасчетныйЛисток_send", ПараметрыВызова, context_params, Неопределено, "Отправка в "+ЛокализацияНазваниеПродукта());
КонецПроцедуры
...
Процедура ОтправитьПрочийКадровыйРасчетныйЛистокВСБИС(ПараметрыВызова) Экспорт
	...
	Если  НЕ ПроверитьПолнотуДанныхОтбораДляВызоваИНИ(ПараметрыВызова) Тогда
		Возврат;
	КонецЕсли;
	Saby_ФоновыеЗаданияКлиент.ЗапуститьINIФоновымЗаданием("ПрочийКадровыйРасчетныйЛисток_send", ПараметрыВызова, Неопределено, Неопределено, "Отправка в "+ЛокализацияНазваниеПродукта());
КонецПроцедуры
...
Процедура ОткрытьЖурналДокументовКЭДОСФильтромПоОбъектам(Команда, ПараметрКоманды ) Экспорт
	...
	КонецЕсли;
	ОткрытьФормуОбработки("ДокументыКЭДО", ДопПараметры, ПараметрКоманды.Форма); 
КонецПроцедуры
