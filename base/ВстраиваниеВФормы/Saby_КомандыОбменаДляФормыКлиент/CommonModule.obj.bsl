...
Процедура ВыполнитьПроизвольныйИНИ(ИмяИНИФайла, ПараметрыВызова) Экспорт	
	МодульФоновогоЗаданияКлиент().ЗапуститьINIФоновымЗаданием(ИмяИНИФайла, ПараметрыВызова, Неопределено, Неопределено, "Загрузка в "+ЛокализацияНазваниеПродукта());	
КонецПроцедуры 
...
Процедура ОтправитьПрочийКадровыйРасчетныйЛистокВСБИС(ПараметрыВызова) Экспорт
	...
	Если  НЕ ПроверитьПолнотуДанныхОтбораДляВызоваИНИ(ПараметрыВызова) Тогда
		Возврат;
	КонецЕсли;
	МодульФоновогоЗаданияКлиент().ЗапуститьINIФоновымЗаданием("ПрочийКадровыйРасчетныйЛисток_send", ПараметрыВызова, Неопределено, Неопределено, "Загрузка в "+ЛокализацияНазваниеПродукта());
КонецПроцедуры
...
Процедура ПолучитьПризнакУчаствуетВКЭДО(Команда, ПараметрКоманды ) Экспорт
	...
	Для Каждого СсылкаСотрудник Из мСсылок.Источник Цикл
		ПредЗначение = Saby_Core.ДанныеПоСотруднику(СсылкаСотрудник);
