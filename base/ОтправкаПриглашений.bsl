...
Функция ПолучитьДанныеПоСотруднику(_Сотрудник) Экспорт
	...
	ПараметрыВызова = Новый Структура("ini_name, object, endpoint, params, connection_uuid", "Blockly_Сотрудники_read", объект_сотр, Неопределено, context_params, context_params["ConnectionId"]);
	...
	Если типИС.Количество() > 1 Тогда
		ТипОбъекта = ТипИС[1];
	КонецЕсли;	
	ПараметрыВызова = Новый Структура("ini_name, object, endpoint, params, connection_uuid", "Blockly_"+ТипОбъекта+"_read", РезультатСотрудник.data["НашаОрганизация"], Неопределено, context_params, context_params["ConnectionId"]);
	...
	Если типИС.Количество() > 1 Тогда
		ТипОбъекта = ТипИС[1];
	КонецЕсли;	
	ПараметрыВызова = Новый Структура("ini_name, object, endpoint, params, connection_uuid", "Blockly_"+ТипОбъекта+"_read", РезультатСотрудник.data["ЧастноеЛицо"], Неопределено, context_params, context_params["ConnectionId"]);
