...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если web_interface = "BrowserSabyPlugin" Тогда
		...
		АдресСтраницы = АдресLPP(АдресСтраницы, "lpp1c.");
	ИначеЕсли web_interface = "BrowserSaby" Тогда
		...
		АдресСтраницы = АдресLPP(АдресСтраницы, "lppbl1c.");
	Иначе
		Префикс = "";
	КонецЕсли;
...
Функция АдресLPP(Адрес, Префикс)
	Адрес = СтрЗаменить(Адрес, "ie-1c.saby", Префикс + "sbis");
	Адрес = СтрЗаменить(Адрес, "ieg-1c.sbis", Префикс + "sbis");
	Адрес = СтрЗаменить(Адрес, "ie-1c.setty", Префикс + "setty");
	Адрес = СтрЗаменить(Адрес, "online.saby", Префикс + "sbis");
	Адрес = СтрЗаменить(Адрес, "online.sbis", Префикс + "sbis");
	Возврат Адрес;
КонецФункции
...
Процедура СформироватьАдресСоСлужебнымиПараметрами()
	...
	Если context_param <> Неопределено Тогда
		...
		Если НеобходимТикет Тогда
			Попытка
			   Тикет = Транспорт.ПолучитьТикетТекущейСессии( context_param ); // если сессия не живая, значит пользователь получит окно аунтентификации
			Исключение
