...
Процедура ПолеHTMLдокументаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ЭлементСобытия["id"] = "toExtSys" Тогда
		...
		Исключение
			//Закомментировано, т.к. падаем в форме Вход при авторизации через браузер
			//ИнфоОбОшибке = ИнформацияОбОшибке();
			//СтруктураОшибки = ExtExceptionAnalyse(ИнфоОбОшибке);
			//Ошибка = ПодготовитьРезультат(СтруктураОшибки);
			//ВернутьОшибку(Ошибка);	
		КонецПопытки;
	КонецЕсли;	
КонецПроцедуры
...
&НаСервере
Функция ПодготовитьРезультат(Знач Результат)
