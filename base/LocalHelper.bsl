...
// BSLLS:NumberOfOptionalParams-on
	...
	Если ПараметрыПоиска <> Неопределено Тогда
		ПараметрыВызоваМетода.Вставить("Search", ПараметрыПоиска);	
	КонецЕсли;
...
Функция local_helper_auth_by_login(context_params, __kwargs = Неопределено) Экспорт
	...
	Попытка
		Возврат local_helper_auth_by_login_password(context_params, context_params.login, context_params.password);
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
...
Функция local_helper_system_info() Экспорт
	...
	Data.Вставить("Product",НазваниеПродукта());
