Процедура ВызватьИсключениеВводаПароляСертификата(context, cert, block_context, СтатусВводаПароля) 
	...
	_ДанныеИсключения.Вставить("multi_thread_loop_variable_scopes", Saby_КопияМассива(context.variable_scopes, context.variable_scope_current + 1));
...
Процедура ОбработатьСтатусВводаПароляСертификата(context, Серт, block_context, СтатусВводаПароля, ИнфОбОшибке)  
	...
	ПричинаОграниченногоРежима = "контекст был получен в ограниченном режиме."; // Для Крипто-Про 5.0
	Если ТребуетсяВвестиПароль = Истина 
		И (Найти(message, ПричинаПарольЗакрытогоКлюча) > 0 
			Или Найти(message, Причина0x80090022) > 0
			Или Найти(message, ПричинаОграниченногоРежима) > 0) Тогда 
		ВызватьИсключениеВводаПароляСертификата(context, Серт, block_context, СтатусВводаПароля);		
	КонецЕсли;	
КонецПроцедуры
...
Функция сбисСписокСертификатовДляАвторизацииНаСервере(ОписанияПрограмм) Экспорт
	...
	Для Каждого Серт из ЛокальныеСертификатыНаСервере Цикл
		Попытка
			...
			СертИНН = get_prop(Серт.Субъект, "OID1_2_643_3_131_1_1");
			СертификатыДляАвторизации.Добавить(Новый Структура("ДвоичныеДанные,Отпечаток,Название,ФИО,ИНН,Должность,ДействителенПо",
																СертДвоичныеДанные, 
																СертОтпечаток,
																Серт.Субъект.CN, 
