...
Функция ВложениеНадоПодписать(Вложение)
	ТребуемоеДействие = get_prop(Вложение, "ТребуемоеДействие");
	Возврат ТребуемоеДействие = Неопределено Или ТребуемоеДействие = "Подписать";	
КонецФункции

//DynamicDirective

Функция ВложенияДляПодписания(ВложенияДокумента) 
	Вложения = Новый Массив;	
	Для каждого _Вложение из ВложенияДокумента Цикл  
		Если ВложениеНадоПодписать(_Вложение) Тогда  
			Вложения.Добавить(_Вложение);
		КонецЕсли;   
	КонецЦикла;
	Возврат Вложения; 
КонецФункции
...
Процедура get_signatures(context, doc, ДанныеДляПодписания, block_context)
	...
	ВложенияДляПодписания = ВложенияДляПодписания(doc["Этап"]["Вложение"]); 
	...
	Если ДанныеДляПодписания[РеквизитТип] = "Дистанционное" Тогда
		...
		КонецЕсли;
		ПодписатьДистанционно(context, ВложенияДляПодписания, ДанныеДляПодписания["СертификатДляПодписания"]);
	КонецЕсли;	
	Если ДанныеДляПодписания[РеквизитТип] = "ЛокальноеНаСервере" Тогда
		ПолучитьДвоичныеДанныеВложений(context, doc);
		ПодписатьНаСервере(context, ВложенияДляПодписания, ДанныеДляПодписания["СертификатДляПодписания"], block_context);
	КонецЕсли;	
	Если ДанныеДляПодписания[РеквизитТип] = "ЛокальноеНаКлиенте" Тогда
		ПодписатьНаКлиенте(context, ВложенияДляПодписания, ДанныеДляПодписания, block_context);
	КонецЕсли;	
	...
	Вложения = Новый Массив;	
	Для каждого _Вложение из ВложенияДляПодписания Цикл
	...
	КонецЦикла; 
	// Добавим вложения, не требуеющие подписания
	Для каждого _Вложение из doc["Этап"]["Вложение"] Цикл 
		Если ВложениеНадоПодписать(_Вложение) Тогда
			Продолжить;
		КонецЕсли;
		ВложениеДанные = Новый Структура("Идентификатор", _Вложение["Идентификатор"]);
		Вложения.Добавить(ВложениеДанные);	
	КонецЦикла;
