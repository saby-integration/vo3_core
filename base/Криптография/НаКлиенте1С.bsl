...
Процедура ОбновитьКэшЛокальныхСертификатов(ОбработчикРезультата, ВремяЖизниКэша = 60) Экспорт
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ОбновитьКэшЛокальныхСертификатовПослеНахожденияМенеджераКриптографии", ЭтаФорма, Контекст);
...
Процедура ОбновитьКэшЛокальныхСертификатовПослеНахожденияМенеджераКриптографии(МенеджерКриптографии,
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ОбновитьКэшЛокальныхСертификатовПослеПолученияХранилищаСертификатов", ЭтаФорма, Контекст);
...
Процедура ОбновитьКэшЛокальныхСертификатовПослеПолученияХранилищаСертификатов(ХранилищеСертификатовКриптографии,
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ОбновитьКэшЛокальныхСертификатовЗавершение", ЭтаФорма, Контекст);
...
Процедура НайтиМенеджерКриптографииНачатьПоиск(Контекст)
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"НайтиМенеджерКриптографииПослеСозданияМенеджеровКриптографии", ЭтаФорма, Контекст);
...
Процедура СоздатьМенеджерыКриптографии(ОбработчикРезультата, Алгоритм = Неопределено)
	...
	ОбработчикСледующегоШага = Новый ОписаниеОповещения(
		"СоздатьМенеджерыКриптографииНачатьСоздание", ЭтаФорма, Контекст);
	...
	Если Контекст.ТекущийИндексПрограммыКриптографии < Контекст.КоличествоПрограммКриптографии Тогда
		...
		СредстваКриптографии.НачатьПолучениеИнформацииМодуляКриптографии(
			Новый ОписаниеОповещения(
				"СоздатьМенеджерыКриптографииПослеПолученияИнформацииМодуляКриптографии", ЭтаФорма, Контекст,
				"СоздатьМенеджерыКриптографииПослеОшибкиПолученияИнформацииМодуляКриптографии", ЭтаФорма),
...
Процедура СоздатьМенеджерыКриптографииНачатьИнициализацию(Контекст)
    ...
    Криптография.НачатьИнициализацию(
        Новый ОписаниеОповещения(
            "СоздатьМенеджерыКриптографииПослеИнициализацииМенеджераКриптографии", ЭтаФорма, Контекст,
            "СоздатьМенеджерыКриптографииПослеОшибкиИнициализацииМенеджераКриптографии", ЭтаФорма),
		СвойстваПрограммы.ИмяПрограммы, СвойстваПрограммы.ПутьКПрограмме, СвойстваПрограммы.ТипПрограммы,
...
Процедура ПодключитьРасширениеРаботыСКриптографиейНаВебКлиентеНачатьПодключение(Контекст) Экспорт
    ...
    НачатьПодключениеРасширенияРаботыСКриптографией(
        Новый ОписаниеОповещения(
			"ПодключитьРасширениеРаботыСКриптографиейНаВебКлиентеПослеПодключенияРасширения", ЭтаФорма, Контекст));
...
Процедура ПодключитьРасширениеРаботыСКриптографиейНаВебКлиентеПослеПодключенияРасширения(Подключено,
	...
	ИначеЕсли Контекст.УстановитьРасширенияРаботыСКриптографиейЕслиНеПодключено Тогда
        ...
        НачатьУстановкуРасширенияРаботыСКриптографией(
            Новый ОписаниеОповещения(
				"ПодключитьРасширениеРаботыСКриптографиейНаВебКлиентеНачатьПодключение", ЭтаФорма, Контекст));
...
Процедура ПолучитьХранилищеСертификатов(ОбработчикРезультата, МенеджерКриптографии)
	...
	МенеджерКриптографии.НачатьПолучениеХранилищаСертификатов(
		Новый ОписаниеОповещения("ПолучитьХранилищеСертификатовЗавершение", ЭтаФорма, Контекст),,);
...
Процедура НайтиСертификатПоОтпечатку(ОбработчикРезультата, ХранилищеСертификатовКриптографии, Отпечаток)
	...
	ХранилищеСертификатовКриптографии.НачатьПоискПоОтпечатку(
		Новый ОписаниеОповещения("НайтиСертификатПоОтпечаткуЗавершение", ЭтаФорма, Контекст),
		Отпечаток);
...
Процедура ПолучитьВсеСертификаты(ОбработчикРезультата, ХранилищеСертификатовКриптографии)
	...
	ХранилищеСертификатовКриптографии.НачатьПолучениеВсех(
		Новый ОписаниеОповещения("ПолучитьВсеСертификатыЗавершение", ЭтаФорма, Контекст));
...
Процедура Подписать(ОбработчикРезультата, Сертификат, Вложения) Экспорт
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ПодписатьПослеПоискаМенеджеровКриптографии", ЭтаФорма, Контекст);
...
Процедура ПодписатьПослеПоискаМенеджеровКриптографии(МенеджерКриптографии, Контекст) Экспорт
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ПодписатьПослеПолученияХранилищаСертификатов", ЭтаФорма, Контекст);
	ПолучитьХранилищеСертификатов(ОбработчикРезультата, МенеджерКриптографии);
...
Процедура ПодписатьПослеПолученияХранилищаСертификатов(ХранилищеСертификатовКриптографии, Контекст) Экспорт
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ПодписатьПослеПоискаСертификатаПоОтпечатку", ЭтаФорма, Контекст);
...
Процедура ПодписатьНачатьПодписаниеСертификатом(Контекст) Экспорт
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ПодписатьПослеПодписанияСертификатом", ЭтаФорма, Контекст);
...
Процедура ПодписатьСертификатом(ОбработчикРезультата, МенеджерКриптографии, ИсходныеДанные, Сертификат)
	...
	МенеджерКриптографии.НачатьПодписывание(
		Новый ОписаниеОповещения(
			"ПодписатьСертификатомЗавершение", ЭтаФорма, Контекст,
			"ПодписатьСертификатомПослеОшибкиПодписания", ЭтаФорма),
