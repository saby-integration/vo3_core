...
Процедура ОбновитьКэшЛокальныхСертификатов(ОбработчикРезультата, ВремяЖизниКэша = 60) Экспорт
	...
	// читаем из хранилища
	КэшЛокальныхСертификатов = Saby_ФоновыеЗаданияСервер.КэшЛокальныхСертификатовПрочитать();
	...
	Saby_ФоновыеЗаданияСервер.КэшЛокальныхСертификатовЗаписать(КэшЛокальныхСертификатов);
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ОбновитьКэшЛокальныхСертификатовПослеНахожденияМенеджераКриптографии", МодульФункции(), Контекст);
...
Процедура ОбновитьКэшЛокальныхСертификатовПослеНахожденияМенеджераКриптографии(МенеджерКриптографии,
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ОбновитьКэшЛокальныхСертификатовПослеПолученияХранилищаСертификатов", МодульФункции(), Контекст);
...
Процедура ОбновитьКэшЛокальныхСертификатовПослеПолученияХранилищаСертификатов(ХранилищеСертификатовКриптографии,
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"ОбновитьКэшЛокальныхСертификатовЗавершение", МодульФункции(), Контекст);
...
Процедура ОбновитьКэшЛокальныхСертификатовЗавершение(Сертификаты, Контекст) Экспорт
	...
	КонецЦикла;
	Для Каждого Менеджер Из РасширениеМенеджерыКриптографии Цикл
