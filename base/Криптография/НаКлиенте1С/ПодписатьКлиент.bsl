...
Процедура ПодписатьПослеПодписанияСертификатом(ПодписанныеДанные, Контекст) Экспорт
	...
	Если ТипЗнч(ПодписанныеДанные) = Тип("Структура")
		...
		ВыполнитьSabyОписаниеОповещения(Контекст.ОбработчикРезультата, ПодписанныеДанные); // Ошибка
		Возврат;
	КонецЕсли;
...
Процедура ПодписатьЗавершение(Контекст)
	...
	ВыполнитьSabyОписаниеОповещения(Контекст.ОбработчикРезультата, Контекст.Результат);
