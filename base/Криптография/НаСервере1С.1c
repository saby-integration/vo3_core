...
Функция СоздатьМенеджерыКриптографииНаСервере(ПрограммыКриптографии, Алгоритм = Неопределено)
	...
	Для Каждого СвойстваПрограммы Из ПрограммыКриптографии Цикл
		...
		Попытка
			Менеджер = Новый МенеджерКриптографии(СвойстваПрограммы.ИмяПрограммы, СвойстваПрограммы.ПутьКПрограмме, СвойстваПрограммы.ТипПрограммы, ИспользованиеИнтерактивногоРежимаКриптографии.Использовать);
		Исключение
			Продолжить;
		КонецПопытки;
