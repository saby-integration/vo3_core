...
Функция ПодписатьДистанционно(context, attachmentList, cert) Экспорт
	Попытка
		...
		operation_uuid = Транспорт.local_helper_init_remote_signing(context.params, sign_dict);
	...
	Попытка
		Для Сч=1 по 60 Цикл
			sign_array = Транспорт.local_helper_get_remote_signature(context.params, operation_uuid);
