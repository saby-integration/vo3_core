...
Процедура ОткрытьИсториюОбменаНажатие(Элемент)
	...
	Иначе
		АдресСтраницы = "https://ie-1c.saby.ru/auth/";	
	КонецЕсли;
	ПараметрыФормыИсторияОбмена = Новый Структура();
	ПараметрыФормыИсторияОбмена.Вставить("Заголовок", "История обмена");
	ПараметрыФормыИсторияОбмена.Вставить("АдресСтраницы", АдресСтраницы);
	ПараметрыФормыИсторияОбмена.Вставить("context_param", context_param);
	ОткрытьФормуОбработки("Browser", ПараметрыФормыИсторияОбмена,,"История обмена");
КонецПроцедуры
