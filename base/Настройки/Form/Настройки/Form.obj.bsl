...
Процедура УстановитьЗаголовокПодсказкуКнопкиВыйти()
	...
	Если АвторизацияВыполнена Тогда
		...
		Если get_prop(saby_param, "session", Новый Структура).Свойство("AccountTitle") Тогда
			ЭлементыФормочки.Аккаунт.Заголовок = get_prop(saby_param, "session", "").AccountTitle;
		Иначе
			ЭлементыФормочки.Аккаунт.Заголовок = "";
		КонецЕсли;
...
Процедура ПрочитатьНастройкиПодсистемы(ИмяПодсистемы, ОтображатьФО = Истина)
	...
	ИмяФО = ПрефиксПродукта() + "Использовать" + ИмяПодсистемы;
	Если Метаданные.ФункциональныеОпции.Найти(ИмяФО) = Неопределено Тогда
		ЭлементыФормочки["Использовать" + ИмяПодсистемы].Видимость = Ложь;
	Иначе
		ЭтаФорма["Использовать" + ИмяПодсистемы] = Константы[ИмяФО].Получить();
	КонецЕсли;
	ЭлементыФормочки["Использовать" + ИмяПодсистемы].Видимость = ЭлементыФормочки["Использовать" + ИмяПодсистемы].Видимость И ОтображатьФО;
...
Процедура ЗаписатьНастройкиПодсистемы(ИмяПодсистемы)
	...
	ИмяФО = ПрефиксПродукта() + "Использовать" + ИмяПодсистемы;
	Если Метаданные.ФункциональныеОпции.Найти(ИмяФО) <> Неопределено Тогда
		Если ИмяПодсистемы = "ЭЛН" И Константы[ИмяФО].Получить() = Ложь И ЭтаФорма["Использовать" + ИмяПодсистемы] = Истина Тогда
			ПриВключенииСЭДО();
		КонецЕсли;
		Константы[ИмяФО].Установить(ЭтаФорма["Использовать" + ИмяПодсистемы]);
	КонецЕсли;
