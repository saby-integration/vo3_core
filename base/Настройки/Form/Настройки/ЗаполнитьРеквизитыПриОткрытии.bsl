...
Процедура ЗаполнитьРеквизитыИзПубличныхНастроек()
	...
	SendMessage = get_prop(ПубличныеОбщиеНастройки, "SendMessage", Неопределено);
	ЭтаФорма.ЭлектроннаяПочтаДляРассылки	= get_prop(SendMessage, "EmailFailAutoOperation", "");
	ЭтаФорма.ПолучатьУведомленияОСбояхАО = Истина;
	Если ПустаяСтрока(ЭтаФорма.ЭлектроннаяПочтаДляРассылки) Тогда
		ЭтаФорма.ПолучатьУведомленияОСбояхАО = Ложь;
	КонецЕсли;
	...
	AlgorithmsStorage = get_prop(ПубличныеОбщиеНастройки, "UseLocalAlgorithmStorage", Неопределено);	
	Если ЗначениеЗаполнено(AlgorithmsStorage) Тогда
		ЭтаФорма.ПИМИспользоватьКопиюИзКаталога = Истина;
		ЭтаФорма.ПИМПутьККаталогу = AlgorithmsStorage;	
	КонецЕсли;
...
Процедура ЗаполнитьДатуСобытияОбновленияСтатусов(context_param)
	...
	Если Не ПустаяСтрока(ДатаСобытияСтрокой) Тогда
		ЭлементыДатыСобытия = Saby_СтрРазделить82(СтрЗаменить(ДатаСобытияСтрокой, " ", "."), ".");
