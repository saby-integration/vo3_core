...
Процедура СкачатьАктуальнуюВерсию(Элемент = Неопределено)
	...
	Если ЗначениеЗаполнено(СсылкаДляСкачивания) Тогда
		...
		#Если ВебКлиент Тогда
			...
			ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(СсылкаДляСкачивания);
		#Иначе	
...
&НаСервере
Процедура ОбновитьИнформациюОВерсииНаСервереПоШаблону(Шаблон, СтатусВерсии)
	ЭлементыФормочки		= ПолучитьЭлементыФормыНаСервере();
	...
	Если ЭлементСписокИзменений = Неопределено Тогда
		ЭлементСписокИзменений = Новый Структура("Видимость");
	КонецЕсли;
	Если Шаблон = 0 Тогда
...
&НаСервере
Процедура ОбновитьИнформациюОВерсииНаСервере(Знач СтатусВерсии)
	ЭлементыФормочки		= ПолучитьЭлементыФормыНаСервере();
	...
	ОбновитьИнформациюОВерсииНаСервереПоШаблону(Шаблон, СтатусВерсии);
...
#Область include_core_base_ЗапускМодуля_ПроверкаВерсии_ПроверитьАктуальность
#КонецОбласти
...
#Область include_core_base_ЗапускМодуля_ПроверкаВерсии_НаФормеОсобенностиПродукта
#КонецОбласти
