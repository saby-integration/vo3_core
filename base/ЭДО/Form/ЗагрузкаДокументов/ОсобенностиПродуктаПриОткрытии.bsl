...
&НаКлиенте
Процедура ЗагрузитьВСБИСДействие(ПроверкаОбмена, context_params)
...
Процедура ЗагрузитьВСБИСОбработкаОтветаНаВопрос(ОтветПользователя, ПараметрКоманды ) Экспорт 
	...
	ЗагрузитьВСБИСДействие(ПроверкаОбмена, get_prop(ПараметрКоманды, "context_params"));
КонецПроцедуры
...
Процедура ВыполнитьПроверкуДокументов(context_params)   
	...
	Если ВопросыПользователю.Количество() = 0 Тогда
		ЗагрузитьВСБИСДействие(ПроверкаОбмена, context_params);		
	Иначе
		ВопросКПользователю = ВопросыПользователю[0].ТекстВопроса;
...
Процедура ПослеПроверкиДокументовНаСервере(мДокументы) 
	...
	ПерезапуститьДОВидимость = НЕ ИсключающийОбъектПерезапускаДО(ПервыйДок.Ссылка);
	...
	Для каждого СтрокаОбъекта Из Объекты Цикл
		...
		Если ЗаписиОбОбъекте <> Неопределено Тогда
			ЭлементыФормочки.ПерезапуститьДО.Видимость = Истина И ПерезапуститьДОВидимость;
		КонецЕсли;
		Прервать;
	КонецЦикла;
