...
Процедура ТипОбъектаПриИзменении(Элемент)
	...
	// Установить тив выгружаемого объекта у списка объектов
	...
	ДокументыКОтправкеПриАктивизацииСтроки(Элемент);
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объекты.Очистить();
	ТипыВыгружаемыхОбъекта.Очистить();
...
Процедура ЗаполнитьВыборТипыВыгружаемогоОбъекта(ВидОбъекта)
	...
	СтрокиТаблицыОбъектов = ТипыВыгружаемыхОбъекта.НайтиСтроки(ОтборСтруктура);
	Если СтрокиТаблицыОбъектов.Количество() = 0 Тогда
		ПредставлениеСБИС = КрасивоеПредставлениеПоТипуСБИС(ВидОбъекта);
		ЭлементыФормочки.ТипОбъекта.СписокВыбора.Добавить(ВидОбъекта, ПредставлениеСБИС);
	Иначе
		Для Каждого СтрокаОбъекта Из СтрокиТаблицыОбъектов Цикл 
			ПредставлениеСБИС = КрасивоеПредставлениеПоТипуСБИС(СтрокаОбъекта.ТипыОбъекта, ВидОбъекта);
			ЭлементыФормочки.ТипОбъекта.СписокВыбора.Добавить(СтрокаОбъекта.ТипыОбъекта, ПредставлениеСБИС);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры	
...
Функция ПолучитьОбрабатываемыеОбъекты()
	...
	Для Каждого ВыгружаемыйДокумент Из Объекты Цикл
		...
		Если ВыгружаемыйДокумент.ВидОбъекта <> ВыгружаемыйДокумент.ТипОбъекта Тогда
			ИмяИни = "СинхВыгрузка_" + ВыгружаемыйДокумент.ВидОбъекта
				+ ТипОбъектаАПИПоТипуСБИС(ВыгружаемыйДокумент.ТипОбъекта);
		КонецЕсли;
		// BSLLS:Typo-on
...
&НаКлиенте
Процедура ЗаполнитьТипВыгружаемыхОбъектов(ВидОбъекта)
	ОтборСтруктура = Новый Структура("ВидОбъекта", ВидОбъекта);
	СтрокиТаблицыОбъектов = ТипыВыгружаемыхОбъекта.НайтиСтроки(ОтборСтруктура);
	Если СтрокиТаблицыОбъектов.Количество() > 0 Тогда
	...
	ТипыОбъекта	= ПолучитьИзОбъекта1СОбъектСБИС(ВидОбъекта);
	Для Каждого ТипВыгружаемгоОбъекта Из ТипыОбъекта Цикл
		НоваяЗапись = ТипыВыгружаемыхОбъекта.Добавить();
		НоваяЗапись.ВидОбъекта = ВидОбъекта;
		НоваяЗапись.ТипыОбъекта = ТипВыгружаемгоОбъекта;
	КонецЦикла;
КонецПроцедуры
