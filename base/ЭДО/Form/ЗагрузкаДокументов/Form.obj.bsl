...
Процедура ЗаполнитьТЗДоступныхРегламентов()
	...
	РегламентовКадровыеПрочие = ПолучитьСписокРегламентов("CadresOther");
	...
	Для Каждого СтрДокум Из Объекты Цикл
		...
		Если РегламентПоУмолчанию <> Неопределено Тогда
			...
			//Проверим наличие регламента по умолчанию в списке
			Если мРегламентыОбъекта.Найти(ВремРегламент) = Неопределено Тогда
				ВремРегламент = ПервыйРегламент;
			КонецЕсли;
		КонецЕсли;
