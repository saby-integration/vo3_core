...
Процедура СписокЗадачКомандаОткрытьВСБИС(Команда)
	...
	Если ТД <> Неопределено И ЗначениеЗаполнено(ТД.СсылкаСБИС) Тогда
		...
		НастройкиПользователя = get_prop(ОбщиеНастройки, "public");
		Если get_prop(НастройкиПользователя, "web_interface", "1CBrowser") = "1CBrowser" Тогда
			ПараметрыФормы = Новый Структура();
			ПараметрыФормы.Вставить("Заголовок", "Кадровые документы");
			ПараметрыФормы.Вставить("АдресСтраницы", ЗадачаСсылка);
			ПараметрыФормы.Вставить("context_param", context_param);
			ОткрытьФормуОбработки("Browser", ПараметрыФормы,, ПараметрыФормы.Заголовок);
		Иначе
			...
			ЗадачаСсылка = СтрЗаменить(ЗадачаСсылка, "-ie-1c.saby", "-lpp1c.sbis");
			ЗадачаСсылка = СтрЗаменить(ЗадачаСсылка, "-ieg-1c.sbis", "-lpp1c.sbis");
			ЗадачаСсылка = Префикс + ЗадачаСсылка;
...
Функция ТекущийДокументИС() 
	Возврат Элементы.СписокЗадач.ТекущиеДанные.Документ1С;
КонецФункции
