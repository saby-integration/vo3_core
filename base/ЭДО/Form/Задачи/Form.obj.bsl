...
Процедура ЗаполнитьСтрокуЗадачиИзЗадачиСбис(СтрокаЗадачи, ЗадачаСбис)
	...
	Для Каждого _Сотрудник Из Сотрудники Цикл
		Подразделение = get_prop(_Сотрудник,"Подразделение" ,"");
...
Процедура СписокЗадачОбработкаВыбораСтроки()
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Регламент", ТекущиеДанные["Этап"]);
...
Процедура СписокЗадачКомандаОткрытьВСБИС(Команда)
	СсылкаСБИС = Элементы.СписокЗадач.ТекущиеДанные.СсылкаСБИС;  
	Если СсылкаСБИС <> Неопределено Тогда
		ЗадачаСсылка = ПолучитьПрямуюССылку(СсылкаСБИС);
