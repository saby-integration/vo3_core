...
Процедура ЗаполнитьСтрокуЗадачиИзЗадачиСбис(СтрокаЗадачи, ЗадачаСбис)
	...
	СостояниеСБИС = Справочники[ПрефиксПродукта() + "СостоянияОбъектов"].ПолучитьСтатусДокументаСБИС(get_prop(Состояние, "Код", 0));
...
Функция ТулбарЗадач(context_params)
	...
	ПараметрыВызова.Вставить("ContextId", Строка(Новый УникальныйИдентификатор())); 
