&НаКлиенте
Перем ДокументСБИС, Транспорт, BlocklyExecutor;
...
Функция ПрочитатьДокумент()
	...
	Возврат Транспорт.ПрочитатьДокумент(context_param, Документ);	
КонецФункции
...
Процедура ИмяИзСБИСвАПИ3(ИмяСБИС, ПримечаниеСБИС, ИдентификаторДокумента)
	...
	ИначеЕсли ИмяСБИС = "Allowance" Тогда
		Если Найти(ПримечаниеСБИС, "уход") > 0 Тогда
			МодульОбъекта = ПолучитьМодульОбъекта();
			context_param = ПроверитьНаличиеПараметровПодключенияНаСервере();
			ДокументСБИС = Транспорт.ПрочитатьДокумент(context_param, Новый Структура("Идентификатор", ИдентификаторДокумента));
