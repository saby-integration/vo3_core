...
Функция ПолучитьСписокДействийСБИС(Документ)
	...
	Если СписокДействий = Неопределено Тогда
		Если context_params = Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ДокументСБИС = ТранспортИнтеграции.local_helper_read_document(context_params, Документ);
...
&НаСервере
Функция ПолучитьПечатныеФормыНаСервере(Знач СсылкаНаОбъект, Вложения = Неопределено, БезСодержимого = Ложь)
