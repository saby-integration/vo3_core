...
Процедура ОтправитьВложенияНаСервере(context_params)
    ...
    Для Каждого ОбновлДок Из Параметры.СписокДокументовСБИС Цикл
		...
		Если ОбновлДок.Документ1С <> Неопределено Тогда
        ...
        Попытка
            Для Каждого ПФ_ Из ВложенияДляДокумента Цикл
                ...
                Если НРег(СуффиксФайла) <> "pdf" Тогда
					ИмяЗапроса = ОбновлДок.Идентификатор+"_"+ПФ_.Файл.Имя;
					СсылкаНаPDF = Сконвертироватьto_pdfa(context_params, ВложениеДок.Файл.ДвоичныеДанные, ВложениеДок.Файл.Имя, ИмяЗапроса); 
                КонецЕсли;    
            ...
            КонецЦикла;
            ИмяЗапроса = "Вложения_"+ОбновлДок.Идентификатор;	
