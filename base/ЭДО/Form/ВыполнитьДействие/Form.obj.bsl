...
Функция ПолучитьСписокДействийСБИС(Документ)
	...
	Если СписокДействий = Неопределено Тогда
		Если context_params = Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ДокументСБИС = Транспорт.local_helper_read_document(context_params, Документ);
...
Функция Saby_Команда_ВыполнитьINI(ПараметрыМетода, ДополнительныеПараметры )
	...
	Если ДокументыМассив = Неопределено Тогда
		//Ошибка
		ЭлементСтатистики	= Транспорт.local_helper_element_err(НаименованиеДействия, ТипСБИС, error_name, error_detail, data, code, ВсегоОбъектов);
 ...
        	ТипСБИС	= get_prop(ДанныеДокументаСБИС, "Тип", "");
			ЭлементСтатистики	= Транспорт.local_helper_element_action(НаименованиеДействия, ТипСБИС, Новый Структура(), 1);
 			МассивУспехов.Добавить(ЭлементСтатистики);
		КонецЦикла;
	КонецЕсли;
	
	Если МассивОшибок.Количество() > 0 Тогда
		Транспорт.local_helper_register_actions(context_params, МассивОшибок);
 	КонецЕсли;
	Если МассивУспехов.Количество() > 0 Тогда
		Транспорт.local_helper_register_actions(context_params, МассивУспехов);
	КонецЕсли;
