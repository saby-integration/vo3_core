...
Процедура ОтправитьВложенияНаСервере(context_params)
    ...
    МодульОбработки    = Обработки.SABY.Создать();
    ...
    Для Каждого ОбновлДок Из Параметры.СписокДокументовСБИС Цикл
        ВидОбъекта = ОбновлДок.Документ1С.Метаданные().Имя;
        ...
        ДокИВложения.Вставить("Идентификатор", ОбновлДок.Идентификатор);
        ДокИВложения.Вставить("Вложение", Новый Массив);
        ВложенияДляДокумента = МодульОбработки.ПолучитьПечатныеФормы(ОбновлДок.Документ1С, ВложенияДляДокумента);
        ...
        Попытка
            Для Каждого ПФ_ Из ВложенияДляДокумента Цикл
                ...
                СуффиксФайла = СтрРазделить82(ПФ_.Файл.Имя,".");
                СуффиксФайла = СуффиксФайла[СуффиксФайла.Количество() - 1];
                
                СсылкаНаPDF = Неопределено;
                Если НРег(СуффиксФайла) <> "pdf" Тогда
                    СсылкаНаPDF = МодульОбработки.local_helper_convert_to_pdfa_to_disk_x(context_params, ВложениеДок.Файл.ДвоичныеДанные, ВложениеДок.Файл.Имя); 
                КонецЕсли;    
                
                Если get_prop(СсылкаНАPDF, "Convert", Ложь) и НРег(СуффиксФайла) <> "pdf" Тогда
                    ВложениеДок.Файл.Удалить("ДвоичныеДанные");
                    ВложениеДок.Файл.Вставить("Ссылка", get_prop(СсылкаНаPDF, "FileId") );
                    ВложениеДок.Файл.Вставить("Имя", get_prop(СсылкаНаPDF, "FileName") );
                    ВложениеДок.Файл.Вставить("ContentType", "application/pdf");
                КонецЕсли;        
                
                ДокИВложения.Вложение.Добавить(ВложениеДок);
            КонецЦикла;
            Результат = МодульОбработки.local_helper_write_attachment(context_params, ДокИВложения);
        Исключение
            ИнфОбОшибке = ИнформацияОбОшибке();
...
Функция Saby_Команда_ВыполнитьINI(ПараметрыМетода, ДополнительныеПараметры )
	...
	Если ДокументыМассив = Неопределено Тогда
		//Ошибка
		ЭлементСтатистики	= МодульОбъекта.local_helper_element_error(context_params, НаименованиеДействия, ТипСБИС, error_name, error_detail, data, code, ВсегоОбъектов);
 ...
        	ТипСБИС	= get_prop(ДанныеДокументаСБИС, "Тип", "");
			ЭлементСтатистики	= МодульОбъекта.local_helper_element_stat(context_params, НаименованиеДействия, ТипСБИС, 1);
 			МассивУспехов.Добавить(ЭлементСтатистики);
		КонецЦикла;
	КонецЕсли;
	
	Если МассивОшибок.Количество() > 0 Тогда
		МодульОбъекта.local_helper_write_error(context_params, НаименованиеДействия, МассивОшибок, error_name, error_detail, data, code, 1);
 	КонецЕсли;
	Если МассивУспехов.Количество() > 0 Тогда
		МодульОбъекта.local_helper_write_stat(context_params, НаименованиеДействия, МассивУспехов, 1);
	КонецЕсли;
