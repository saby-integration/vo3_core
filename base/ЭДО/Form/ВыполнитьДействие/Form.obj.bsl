...
Процедура ОтправитьВложенияНаКлиенте(context_params)
    ...
    Для Каждого ОбновлДок Из Параметры.СписокДокументовСБИС Цикл
        ...
        Попытка
			...
			Документ1СОбновлДок = get_prop(ОбновлДок, "Документ1С");
			Идентификатор1С = "";
			Если Документ1СОбновлДок <> Неопределено Тогда
				ВидОбъекта = ИмяМетаданных(Документ1СОбновлДок);
				Идентификатор1С = СокрЛП(Документ1СОбновлДок.УникальныйИдентификатор());
			Иначе			
            ...
            ИмяЗапроса = "Вложения_" + Идентификатор;
				...
				_Документ.Вставить("НаименованиеДокумента", Строка(Документ1СОбновлДок));
				_Документ.Вставить("ИдентификаторSaby", Строка(ОбновлДок.Идентификатор));
				_Документ.Вставить("Вложение", ДокИВложения.Вложение);
				_Документ.Вставить("ИмяИС", ВидОбъекта);
				_Документ.Вставить("ИдИС", Идентификатор1С);
	...
	// Запись вложений и обновление статискики
		...
		Контекст.Вставить("НазваниеОперации", "Запись вложения в " + ЛокализацияНазваниеПродукта());
...
&НаКлиенте
Процедура ОповеститьОбИзмененияхДокументов1С() 
	ПараметрыВызоваДокументы = get_prop(ПараметрыВызова, "Документы");
	Если ТипЗнч(ПараметрыВызоваДокументы) <> Тип("Массив") И ПараметрыВызоваДокументы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;	
	Документ1С = get_prop(ПараметрыВызоваДокументы[0],"Документ1С");
	Если ЗначениеЗаполнено(Документ1С) Тогда
    	ОповеститьОбИзменении(Документ1С);
	КонецЕсли;	
...
Процедура ОбработатьРезультатВыполненияINI(Результат)
	...
	ОповеститьОбИзмененияхДокументов1С();
КонецПроцедуры	
...
Функция Saby_Команда_ВыполнитьINI(ПараметрыМетода, ДополнительныеПараметры )
	...
	ПараметрыВызоваБлокли.Вставить("ContextId", get_prop(ДополнительныеПараметры, "ContextId", Неопределено));
	...
	ВидОбъекта = Неопределено;
	Если ini_name	= "Blockly_Документы_execute" Тогда
		...
		EndpointArgs.Вставить("UpdateObjFromIS", UpdateObjFromIS);
...
Процедура ПоказатьРезультатВыполнения(ОбработчикРезультата, Результат, context_params, ПараметрыДействия)
	...
	Если ТипЗнч(Результат) = Тип("Структура") или ТипЗнч(Результат) = Тип("Соответствие") Тогда
		...
		Если Результат.Status = "run" Тогда
			...
			ОбработчикСледующегоШага = SabyОписаниеОповещения("ПерезапуститьВыполнениеНаСервереПослеВыполненияКомандыНаКлиенте", ЭтотОбъектНаКлиенте(), Контекст);
...
Процедура ПерезапуститьВыполнениеНаСервереПослеВыполненияКомандыНаКлиенте(commands_result, Контекст) Экспорт
	...
	Контекст.ПараметрыДействия.Вставить("ContextId", 		Контекст.Результат.ContextId);

	Контекст.Результат = ВыполнитьДействие("Blockly_Документы_execute", Контекст.context_params, Контекст.ПараметрыДействия);
...
Функция ЭтапыДокумента(Документ)
	...
	ИдентификаторДокумента = get_prop(Документ, "ДокументСБИС", get_prop(Документ, "Идентификатор", get_prop(Документ, "ИдСБИС")));
    ИдентификаторДокумента = get_prop(ИдентификаторДокумента, "Идентификатор", ИдентификаторДокумента);
	ДокументСБИС = ТранспортИнтеграции.local_helper_read_document(context_params, Новый Структура("Идентификатор,ДопПоля", ИдентификаторДокумента, ""));
