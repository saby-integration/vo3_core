...
Процедура КомандаВыполнитьДействие(Команда)
	...
	ОбработчикРезультата = Новый ОписаниеОповещения(
		"КомандаВыполнитьДействиеПослеОбновленияКэшаЛокальныхСертификатов", ЭтотОбъектНаКлиенте(), Контекст);
...
Процедура УдалитьДействия()
	...
	Для СчётКоманд = 1 По ВсегоКоманд Цикл 
		ИмяДействия = "Действие_" + Формат(СчётКоманд, "ЧГ=0"); 
		КнопкаДействия = ГруппаДействийДокумента.ПодчиненныеЭлементы.Найти("Кнопка_" + ИмяДействия);
		Если КнопкаДействия = Неопределено Тогда
			Прервать;
		КонецЕсли;
		Элементы.Удалить(КнопкаДействия);
		//МассивКнопок.Добавить(КнопкаДействия);
		ДействиеКоманда = ЭтаФорма.Команды.Найти(ИмяДействия); 
		ЭтаФорма.Команды.Удалить(ДействиеКоманда);		
	КонецЦикла;	
...
Процедура	ПоказатьСписокТекущихЭтапов(ТекущиеЭтапы)
	...
	Элем.ТекущийЭтап.КнопкаВыпадающегоСписка = Истина;
