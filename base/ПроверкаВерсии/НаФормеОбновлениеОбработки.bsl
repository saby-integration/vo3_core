...
&НаСервере
Функция СкачатьФайлИПолучитьАдресДанныхНаСервере(Знач УРЛДляСкачивания)
...
&НаКлиенте
Процедура СбисОбновитьОбработкуФайлВнешнейОбработки(сбисПолноеИмяОбработки,АдресДанныхФайла, НомерАктуальнойВерсии, СообщениеОПерезапускеВКаталогеНаДиске)
	...
	Исключение
		Сообщить("Ошибка при удалении временного файла обработки " + ИмяВременногоФайлаНовойВерсии + ". Подробно: " + ОписаниеОшибки());
	КонецПопытки;
...
Процедура СбисОбновитьОбработкуНаКлиенте(Элемент = Неопределено)
	...
	Если СтатусВерсии.Шаблон = 0 Или СтатусВерсии.Шаблон = "Ошибка" Или СтатусВерсии.СсылкаДляСкачивания = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Отказ = Ложь;
	сбисПолноеИмяОбработки = ПолноеИмяФайлаОбработки();
	ЭтоУправляемоеПриложение = (ТипЗнч(ЭтаФорма) = Тип("УправляемаяФорма"));
	сбисРежимИспользованияОбработки = РежимИспользованияОбработки(сбисПолноеИмяОбработки);
	...
	ИначеЕсли сбисРежимИспользованияОбработки = "Файл внешней обработки" Тогда
		//Обновим файл
		СбисОбновитьОбработкуФайлВнешнейОбработки(сбисПолноеИмяОбработки,АдресДанныхФайла, СтатусВерсии.НомерАктуальнойВерсии, СообщениеОПерезапускеВКаталогеНаДиске);
	Иначе
		Возврат;
	КонецЕсли;
...
//
Функция РежимИспользованияОбработки(ПолноеИмяОбработки) Экспорт
...
//
&НаСервере
Функция ПолноеИмяФайлаОбработки() Экспорт
...
Функция ПолучитьОбработкуСБИСНаСервере(Знач НаименованиеВнешнейОбработкиСбис)
...
&НаСервереБезКонтекста
Функция ЗаписатьОбработкуВСправочник(Знач Версия, Знач ОбработкаСБИССсылка, Знач АдресДанныхФайла, Отказ)
