		...
		Если Файл.Существует() Тогда
			...
			Если Не Отказ Тогда
				Попытка                           	
					...
					СообщениеОПерезапускеВКаталогеНаДиске = СтрЗаменить(СообщениеОПерезапускеВКаталогеНаДиске, "%НомерНовойВерсии%", СтатусВерсии.НомерАктуальнойВерсии);
...
Функция сбисПолноеИмяФайла() Экспорт
	ОбработкаОбъектМодуль = ПолучитьМодульОбъекта();
	Попытка
