...
//  version_info - Структура - информация о версии.
//
...
Функция ПолучитьСтатусВерсии(Принудительно=Ложь, version_info=Неопределено) Экспорт
	...
	Если version_info=Неопределено Тогда  // обработки получают информацию о статусе версии снаружи
		...
		Попытка
			version_info = ТранспортИнтеграции.local_helper_get_product_version_status(context_param, ПолучитьНазваниеПродукта(), ИмяФайлаИНомерВерсии[1]);	
		Исключение
