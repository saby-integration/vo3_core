...
Процедура СоздатьДополнительноеСвойствоСтатусФизЛица()
	ИмяСвойства	= "СтатусФизЛица";
	...
	Если НЕ ЗначениеЗаполнено(НайденноеСвойство) Тогда
		...
		ТипСвойства			= Новый ОписаниеТипов("Строка");
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НДРС.ИмяПредопределенногоНабора = ""Справочник_ФизическиеЛица"" ";
	...
	Если РазделИнтерфейса = Неопределено Тогда
		...
		РазделИнтерфейса.Наименование	= "Физические лица";
		РазделИнтерфейса.Используется	= Истина;
		РазделИнтерфейса.ИмяПредопределенногоНабора	= "Справочник_ФизическиеЛица";
		РазделИнтерфейса.КоличествоСведений	= 1;
	...
	Если мЭлементов.Количество() = 0 Тогда
		...
		СтрокаДопСведений.ИмяПредопределенногоНабора	= "Справочник_ФизическиеЛица";
		РазделИнтерфейса.Записать();
	КонецЕсли;
	...
	Если НЕ ИспользованиеСвИДопЗнч.Значение Тогда
