...
&НаСервере
Функция ТипОбъектаМетаданных(ССылкаНаОбъект) Экспорт
...
Функция ПривестиТипИСКЕдЧислу(ТипИС) Экспорт
	мТипИС = Saby_СтрРазделить82(ТипИС,".");
	
	Если мТипИС.Количество() < 2 Тогда
		Возврат ТипИС;
	КонецЕсли;	
	
	СоответствиеТипов = Новый Соответствие;
	СоответствиеТипов.Вставить("Справочники", "Справочник");
	СоответствиеТипов.Вставить("Документы", "Документ");
	СоответствиеТипов.Вставить("ПланыВидовРасчета", "ПланВидовРасчета");
	СоответствиеТипов.Вставить("Перечисления", "Перечисление");
	
	Если Не СоответствиеТипов.Получить(мТипИС[0]) = Неопределено Тогда
		мТипИС[0] = СоответствиеТипов[мТипИС[0]];
	КонецЕсли;	 
	
	Возврат мТипИС[0] + "." + мТипИС[1];
КонецФункции
