...
Функция ПолучитьПодключение(context_param, НоваяИнтеграция = Ложь) Экспорт
	...
	Попытка
		...
		КонецЕсли;
		Если ConnectionStateEvents.Найти("NewExtSysVersion") <> Неопределено Тогда
			СброситьКэшНаСервере(); // Сбрасываем кэш инишек при смене версии каталога файлов настроек
		КонецЕсли;
		Если ConnectionStateEvents.Найти("ToggledDisableCustomAlgorithms") <> Неопределено Тогда
			СброситьКэшНаСервере(); // Сбрасываем кэш инишек при отключении/включении пользовательских ПИМ
		КонецЕсли;
		context_param.Вставить("Integration", get_prop(connection_info, "Integration"));
		context_param.Вставить("IntegrationId", get_prop(connection_info, "IntegrationId"));
		context_param.Вставить("ConnectionUuid", get_prop(connection_info, "ConnectionId"));
		context_param.Вставить("Connection", get_prop(connection_info, "Connection"));
	...
	Исключение
		...
		context_param.Вставить("IntegrationId", "");
		context_param.Вставить("ConnectionUuid", "");
