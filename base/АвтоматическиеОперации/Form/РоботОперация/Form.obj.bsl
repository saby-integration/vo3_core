...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.КопироватьРасширеннаяПодсказка.Заголовок = "Копировать в буфер обмена: "+ИдентификаторНовый;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если Модифицированность = ИСТИНА Тогда
		...
		Отказ = ИСТИНА;
		ОповещениеПослеВыбораОтветаПередЗакрытием = Новый ОписаниеОповещения("ПослеВыбораОтветаПередЗакрытием",ЭтотОбъект);
...
Процедура АлгоритмНажатие(Элемент, СтандартнаяОбработка)
	...
	ОписаниеЗакрытиеОкна = Новый ОписаниеОповещения("ПослеВыбораАлгоритма",ЭтотОбъект);
	Попытка   
		ИмяФормыВыборАлгоритма = ТипМетаданныхОбработки()+".SABY.Форма.РоботВыборАлгоритма";
		ОткрытьФорму(ИмяФормыВыборАлгоритма,ПараметрыПередачи,,,,,ОписаниеЗакрытиеОкна);
	Исключение  
...
Процедура ДеревоНастроекПодключенияПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.СписокВыбора.Очистить();
		Если ТекущиеДанные.НаправленияВозможные = 0 Тогда
			Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.ТолькоПросмотр = ИСТИНА;
		Иначе
			ШрифтСтрокДерева = Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.Шрифт;
			Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.ТолькоПросмотр = Ложь;
			Если ТекущиеДанные.НаправленияВозможные = 2 Тогда   
				СтрокаСостояния = "⇄  Синхронизировать в обе стороны";
				Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.СписокВыбора.Добавить(2,СтрокаСостояния);	
			КонецЕсли;
			
			Если ТекущиеДанные.НаправленияВозможные = 3
				ИЛИ ТекущиеДанные.НаправленияВозможные = 2 Тогда
				СтрокаСостояния = "→ В СБИС";
				Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.СписокВыбора.Добавить(3,СтрокаСостояния);	
			КонецЕсли;
			
			Если ТекущиеДанные.НаправленияВозможные = 4 
				ИЛИ ТекущиеДанные.НаправленияВозможные = 2 Тогда
				СтрокаСостояния = "← в ИС";
				Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.СписокВыбора.Добавить(4,СтрокаСостояния);	
			КонецЕсли;	
			СтрокаСостояния = "⤱ Отключено";
			Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.СписокВыбора.Добавить(1,СтрокаСостояния);
...
Процедура Редактировать(Команда)
	Если Алгоритм <> "<выберите алгоритм>" Тогда   
		...
		АдресСтраницы = context_param.api_url+"/blockly_editor/page/?id="+Идентификатор+"&connector=1C&command=1C&endpoint=&ini="+НазваниеАлгоритма;
		...
		ОткрытьФорму(ИмяФормыBrowser, ПараметрыФормы,,"Редактор файлов настроек");
	Иначе
...
Процедура Запуск(Команда)
	...
	Если Модифицированность = Истина Тогда  
		ПослеВыбораСохраненияАлгоритма = Новый ОписаниеОповещения("ПослеВыбораСохраненияАлгоритма", ЭтотОбъект);
...
Процедура Копировать(Команда)    
	...
	#Иначе  
		ОписаниеПослеОтображенияID = Новый ОписаниеОповещения("ПослеВводаСтрокиБуфераОбмена",ЭтотОбъект);
...
Процедура ПредставлениеФильтровНажатие(Элемент, СтандартнаяОбработка)
	...
	ОписаниеОЗакрытииОкна = Новый ОписаниеОповещения("ПослеНастройкиФильтров", ЭтотОбъект);
	ИмяФормыРоботПарамФильтрОбъектовСинх = ТипМетаданныхОбработки() + ".SABY.Форма.РоботПарамФильтрОбъектовСинх";
	ОткрытьФорму(ИмяФормыРоботПарамФильтрОбъектовСинх,ПараметрыФормыФильтров,,,,,ОписаниеОЗакрытииОкна,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПротоколОбменаHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Попытка 
		...
		Если ЗначениеЗаполнено(АдресСсылки) Тогда
			...
			ИмяФормыBrowser = ТипМетаданныхОбработки()+".SABY.Форма.Browser";
			ОткрытьФорму(ИмяФормыBrowser, ПараметрыОткрытияОкнаБраузера);
		КонецЕсли;	
	Исключение
	КонецПопытки;	
КонецПроцедуры
...
Процедура ИсторияОбменаТабличныйДокументПриАктивизации(Элемент)
	ТекВерх = Элементы.ИсторияОбменаТабличныйДокумент.ТекущаяОбласть.Верх;
    ...
    МассивОбластей.Добавить(ЯчейкаСтрока);
  	Элементы.ИсторияОбменаТабличныйДокумент.УстановитьВыделенныеОбласти(МассивОбластей);
КонецПроцедуры
...
Процедура ИсторияОбменаТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Область.Расшифровка) Тогда
		...
		ПараметрыОткрытияОкнаБраузера.Вставить("Заголовок", "Страница синхронизации"); 
		ИмяФормыBrowser = ТипМетаданныхОбработки()+".SABY.Форма.Browser";
		ОткрытьФорму(ИмяФормыBrowser, ПараметрыОткрытияОкнаБраузера,,ИСТИНА);	
	КонецЕсли;
КонецПроцедуры
...
Процедура ДеревоНастроекПодключенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ДеревоНастроекПодключенияНаправление" Тогда
		СтандартнаяОбработка = ЛОЖЬ; 
		ПослеВыбораЗначения = Новый ОписаниеОповещения("ПослеВыбораЗначения", ЭтотОбъект);
		СписокВыбора = Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.СписокВыбора;
		//ШрифтСтрокДерева = Элементы.ДеревоНастроекПодключения.ПодчиненныеЭлементы.ДеревоНастроекПодключенияНаправление.Шрифт;
	...
	Если НЕ ЗначениеЗаполнено(УзелПланаОбмена.Код) Тогда
		...
		Для СЧ = 0 По 100 Цикл	
			Попытка 
				...
				НовыйУзел.Наименование = "СБИС "+НаименованиеПодключения;
...
Процедура ЗаполнитьФормуПоПараметрамАлгоритма()
	...
	Если ТипЗнч(ПараметрыАлгоритма) = Тип("Соответствие") Тогда
		Если ПараметрыАлгоритма.Получить("ЭтоСинхронизация") = Истина Тогда
			Элементы.ГруппаОбъектыОбмена.Видимость = ИСТИНА;
			ЭтоСинхронизация = ИСТИНА;
			Элементы.ГруппаПараметрыФильтры.Видимость = ИСТИНА;
			Элементы.ГруппаИсторияОбмена.Видимость = ИСТИНА;
			НазваниеПланаОбмена = ПараметрыАлгоритма.Получить("ПланОбмена");
			ЗаполнитьДеревоНастроек();
		Иначе
			Элементы.ГруппаОбъектыОбмена.Видимость = ЛОЖЬ;
			ЭтоСинхронизация = ЛОЖЬ;
			Элементы.ГруппаПараметрыФильтры.Видимость = ЛОЖЬ; 	
			Элементы.ГруппаИсторияОбмена.Видимость = ЛОЖЬ;
		КонецЕсли;
	КонецЕсли;	
...
Функция ПолучитьПараметрыАлгоритма()
	...
	ПараметрыВызова.Вставить("ini_name",Алгоритм);
...
Процедура ОбновитьОтображениеОграничениеВремени() 
	Элементы.ГруппаВремяЗапуска.Видимость 	= ЛОЖЬ;	
	Элементы.ГруппаБезОграничений.Видимость = ЛОЖЬ;
	Элементы.ГруппаРазВДень.Видимость 		= ЛОЖЬ;
	Если ПериодичностьДня = "24:00" Тогда
		Элементы.ГруппаРазВДень.Видимость = ИСТИНА;
	Иначе	
		Элементы.ГруппаВремяЗапуска.Видимость = НЕ БезОграниченийВремени;	
		Элементы.ГруппаБезОграничений.Видимость = БезОграниченийВремени;
	КонецЕсли;	
КонецПроцедуры
...
Процедура ОтобразитьЦветКнопкиДняНедели(ИмяКоманды, ЗначениеПараметра) 
	Если ЗначениеПараметра = ЛОЖЬ Тогда
		Элементы[ИмяКоманды].ЦветТекста = ШрифтНеактивногоЭлемента;
		Элементы[ИмяКоманды].Пометка = ЛОЖЬ;
	Иначе
		Элементы[ИмяКоманды].Пометка = ИСТИНА;
		Элементы[ИмяКоманды].ЦветТекста = ШрифтАктивногоЭлемента; 
	КонецЕсли;	
КонецПроцедуры	
...
Процедура ЗаполнитьАвтоматическийЗапуск() 
	...
	Если Периодичность = 1 Тогда
		Элементы.ГруппаАвтоЗапускаДниНедели.Видимость = ЛОЖЬ;
	Иначе
		Элементы.ГруппаАвтоЗапускаДниНедели.Видимость = ИСТИНА;
	КонецЕсли;	
...
Функция ИзменитьНаправленияСинхронизации()
	Если ЭтоСинхронизация Тогда 
		...
		ДеревоЗначений = РеквизитФормыВЗначение("ДеревоНастроекПодключения");
...
Процедура ЗаполнитьДеревоНастроек()
	...
	Если ТаблицаНастроек.Количество() > 0 Тогда
		Для Каждого Запись ИЗ ТаблицаНастроек Цикл
			Если Запись.Parent_ = ИСТИНА Тогда
				НоваяВетка = ДеревоЗначений.Строки.Добавить();
				НоваяВетка.Объект = Запись.ObjectName;
			Иначе	
			    НоваяСтрокаВетки = НоваяВетка.Строки.Добавить();
				НоваяСтрокаВетки.Объект 				= Запись.ObjectName;
				НоваяСтрокаВетки.Направление 			= Запись.SyncStatus;
				НоваяСтрокаВетки.НаправленияВозможные 	= РазобратьНаправлениеПоУмолчанию(Запись.PossibleStatus);
				НоваяСтрокаВетки.ИмяОбъекта 			= Запись.ObjId;
			КонецЕсли;	
		КонецЦикла;	
	КонецЕсли;
...
Функция ЗаполнитьРеквизитыПоОперации()
	...
	Если Статус <> Неопределено И Статус > 0 И Статус <> 100 Тогда  
		Элементы.ДекорацияСтатусОшибка.Видимость = Истина;
		Элементы.ДекорацияСтатусВыполнено.Видимость = ЛОЖЬ;
	Иначе
		Элементы.ДекорацияСтатусВыполнено.Видимость = Истина;
		Элементы.ДекорацияСтатусОшибка.Видимость = ЛОЖЬ;
	КонецЕсли;
...
Процедура ОбновитьАвтозапуск()
	Если Автозапуск = Истина Тогда
		Элементы.ГруппаАвтоЗапускаВложенная.Видимость = Истина;
		Элементы.ГруппаДатаВремяСледующегоЗапуска.Видимость = ИСТИНА;
	Иначе
		Элементы.ГруппаАвтоЗапускаВложенная.Видимость = ЛОЖЬ;
		Элементы.ГруппаДатаВремяСледующегоЗапуска.Видимость = ЛОЖЬ;
	КонецЕсли;
...
Процедура ПослеВыполненияАлгоритма(Результат, ДополнительныеПараметры) Экспорт
	...
	Элементы.Запуск.Видимость = ИСТИНА;
...
Функция РаспределениеКубовИсторииОбмена(ЗаписьИсторииОбмена, ВсегоКубов)
	...
	ДобавитьСтрокуТаблицыРаспределения(ТаблицаРаспределения, ЗаписьИсторииОбмена, "SendPercent");
...
Процедура ЗаполнитьИсториюОбмена()   
	...
	МакетИсторияОбмена = Обработка.ПолучитьМакет("ИсторияОбмена");
	...
	Для Каждого Запись ИЗ ТаблицаИсторииОбмена Цикл  
		...
		ВывестиБлокиВТабличныйДокумент("SendPercent", ТаблицаРаспределенияКубов, ТабличныйДокумент, КолонкаСиний); 	
...
Функция ПересчитатьДатуСледующегоЗапуска()
	...
	Если ДатаВремяЗапуска = "" Тогда 
		Элементы.ДатаВремяСледующегоЗапуска.Видимость = ЛОЖЬ; 
	Иначе
		Элементы.ДатаВремяСледующегоЗапуска.Видимость = ИСТИНА; 	
	КонецЕсли;	
...
Процедура ЗапускАлгоритма() 
	...
	ОповещениеОЗакрытииФормыФоновогоЗадания = Новый ОписаниеОповещения("ПослеВыполненияАлгоритма", ЭтотОбъект);
	...
	Элементы.Запуск.Видимость = ЛОЖЬ;
...
Процедура ПослеВыбораЗначения(Результат, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(Результат) = Тип("ЭлементСпискаЗначений")  Тогда
		Элементы.ДеревоНастроекПодключения.ТекущиеДанные.Направление = Результат.Значение;	
		Модифицированность = ИСТИНА;
 	КонецЕсли;	
КонецПроцедуры
...
Процедура ДеревоНастроекПодключенияОтметкаПриИзменении(Элемент)
	ТекущиеДанные = Элементы.ДеревоНастроекПодключения.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда 
...
Процедура УстановитьНаправление(Команда)
	ПослеВыбораВариантаУстановкиНаправления = Новый ОписаниеОповещения("ПослеВыбораВариантаУстановкиНаправления", ЭтотОбъект);
	СписокВыбора = Новый СписокЗначений;
	СписокВыбора.Добавить("2","⇄  В обе стороны");
	СписокВыбора.Добавить("3","→ В СБИС");
	СписокВыбора.Добавить("4","← В ИС");
	СписокВыбора.Добавить("1","⤱  Отключить");
	ЭлементУстановитьСнятьФлажки = Элементы.ДеревоНастроекПодключенияУстановитьНаправление;
...
Процедура ПоказатьСкрытьОшибкуПолученияДанных(Показать, ТекстОшибки) 
	Элементы.ДекорацияОшибкаПолученияДанных.Видимость = Показать;
	Элементы.ДекорацияОшибкаПолученияДанных.Заголовок = ТекстОшибки;	
КонецПроцедуры
