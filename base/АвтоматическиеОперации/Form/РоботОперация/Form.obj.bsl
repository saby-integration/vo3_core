...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры <> Неопределено Тогда 
		...
		ЭтаФорма.Версия = get_prop(Параметры, "Версия", МодульОбъекта.ПреобразоватьВерсию(Метаданные.Версия));	
...
Процедура ПриОткрытии(Отказ)
	...
	Если Идентификатор = ИдентификаторНовый Тогда
		ДанныеФильтра.Вставить("Значения", ЗначенияФильтровПодключения());
	Иначе
		ДанныеФильтра.Вставить("Значения", Новый Соответствие);	
	КонецЕсли;	
...
Функция ЗначенияФильтровПодключения()
	...
	КонецЕсли;
	Если ТипЗнч(ФильтрЗначения) <> Тип("Структура") 
		И ТипЗнч(ФильтрЗначения) <> Тип("Соответствие") Тогда
...
Процедура ЗапускПриСохранении()
	...
	ПараметрыВызова.Вставить("connection_uuid", Идентификатор);
...
Процедура Редактировать(Команда)
	Если Алгоритм <> "<выберите алгоритм>" Тогда   
		...
		АдресСтраницы = АдресСтраницы + "/blockly_editor_1c/page/?connection_id=";
...
Функция ПодготовитьФильтр()
	ТипыСтрокаЧисло = Новый Массив;
	ТипыСтрокаЧисло.Добавить(Тип("Строка"));
	ТипыСтрокаЧисло.Добавить(Тип("Число"));
	...
	Для Каждого Запись Из ДанныеФильтра.Значения Цикл
		...
		ИначеЕсли ТипЗнч(Запись.Значение) = Тип("СписокЗначений") Тогда
			...
			КонецЦикла;
		ИначеЕсли ТипыСтрокаЧисло.Найти(ТипЗнч(Запись.Значение)) = Неопределено Тогда
