...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.ШрифтНеактивногоЭлемента = ЦветаСтиля.ЦветТекстаКнопки;
...
Функция InitConnection()
	МодульОбъекта = МодульОбъекта(); 	
	res = МодульОбъекта.ПолучитьПодключение(context_params);
	Возврат res;
КонецФункции
...
Функция ЗаписатьПодключение() 
	...
	props.Вставить("update_date"		,ТекущаяДатаСеанса());
	...
	NextRun = МодульОбъекта().УниверсальноеВремяСтрокой(NextRun);
	Если Автозапуск = Истина Тогда
	...
	res = МодульОбъекта.local_helper_write_connection(context_params, params);
...
Процедура ИзменитьНаправленияСинхронизации()
	...
	Для Каждого ЗаписьНаправление ИЗ ТаблицаНаправлений Цикл	
		...
		Если МассивДляЗаписи.Количество() > 0 Тогда  	
			...
			МодульОбъекта.local_helper_api3_typesyncobjectmasssetdirection(context_params, params);
		КонецЕсли;	
	КонецЦикла;
КонецПроцедуры	
...
Процедура ЗаполнитьДеревоНастроек()
	...
	ТаблицаНастроек = МодульОбъекта.НаправленияСинхронизации(context_params, Идентификатор);
...
Процедура ЗаполнитьРеквизитыПоОперации()
    ...
    АвтоматическаяОперация = МодульОбъекта.local_helper_read_connection(context_params, ПараметрыОперации);
	...
	Если ЗначениеЗаполнено(LastRun) Тогда
		Этаформа.ДатаВремяПоследнегоЗапуска = МодульОбъекта.ПреобразоватьСтрокуВДатуСЧасовымПоясом(LastRun);
	КонецЕсли;
	NextRun = АвтоматическаяОперация.Получить("NextRun"); 
	Если ЗначениеЗаполнено(NextRun) Тогда
		Этаформа.ДатаВремяСледующегоЗапуска = МодульОбъекта.ПреобразоватьСтрокуВДатуСЧасовымПоясом(NextRun);
	КонецЕсли;
	...
	Этаформа.dataСтрокой = МодульОбъекта.encode_xdto_json(Data);
...
Процедура ЗаполнитьИсториюОбмена() 
	...
	Пока ИСТИНА Цикл
		res = МодульОбъекта.local_helper_api3_extsyncdoc_list(context_params, Фильтр, Неопределено, Неопределено, СчетчикСтраниц);
