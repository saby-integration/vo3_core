...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры <> Неопределено Тогда 
		Сервис = get_prop(Параметры, "Сервис", "Dom1C");	
		Версия = get_prop(Параметры, "Версия", МодульОбъекта.ПреобразоватьВерсию(Метаданные.Версия));	
		Подсистема = get_prop(Параметры, "Подсистема", ""); 	
		context_param = get_prop(Параметры, "context_param", Неопределено); 	
		Идентификатор = get_prop(Параметры, "Идентификатор", ""); 	
		ИдентификаторНовый = get_prop(Параметры, "Идентификатор", "");	
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Идентификатор) Тогда
		ЗаполнитьРеквизитыПоОперации();
	Иначе  
		ИдентификаторНовый = get_prop(Параметры, "Ключ", Строка(Новый УникальныйИдентификатор));
...
Процедура ЗаполнитьРеквизитыПоОперации()
	...
	Если ЗначениеЗаполнено(LastRun) Тогда
		ДатаВремяПоследнегоЗапуска = МодульОбъекта.ПреобразоватьСтрокуВДатуСЧасовымПоясом(LastRun);
	КонецЕсли;
	NextRun = АвтоматическаяОперация.Получить("NextRun"); 
	Если ЗначениеЗаполнено(NextRun) Тогда
		ДатаВремяСледующегоЗапуска = МодульОбъекта.ПреобразоватьСтрокуВДатуСЧасовымПоясом(NextRun);
	КонецЕсли;
...
Функция РазобратьНаправлениеПоУмолчанию(ВозможныеНаправления)
	...
	Для Каждого ЗначениеМассива Из ВозможныеНаправления Цикл
		...
		КонецЕсли;	
		Попытка 
			ЗначениеЧислом = Число(ЗначениеМассива.Получить("key"));
		Исключение  
			Продолжить;
		КонецПопытки; 
		Если ЗначениеЧислом = 2 Тогда
			Возврат ЗначениеЧислом;
		ИначеЕсли ЗначениеЧислом > ТекущееЗначение Тогда
