...
#Область includr_core_base_АвтоматическиеОперации_Form_РоботВыборАлгоритма_ОсобенностиПриложения
#КонецОбласти
...
Процедура ОткрытьФормуРедактора(Алгоритм = Неопределено)
	...
	Если ЗначениеЗаполнено(Алгоритм) Тогда
		...
		ДополнениеАдресаСтраницы = "&algorithm="+НазваниеАлгоритма;
	КонецЕсли;
	connector = КоннекторLPP(context_params);
	#Если ВебКлиент Тогда 
		connector="1C2";	
	#КонецЕсли	 
	АдресСтраницы = context_params.ApiUrl + "/blockly_editor_1c2/page/?connection_id=" + Идентификатор + "&connector=" + connector;
	АдресСтраницы = АдресСтраницы + "&command=" + connector + ДополнениеАдресаСтраницы;	
...
#Область include_core_base_Helpers_ConnectorLPP //&НаКлиентеНаСервереБезКонтекста
#КонецОбласти
