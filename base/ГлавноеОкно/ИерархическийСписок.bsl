...
Процедура ПроставитьПометкиВниз(ТекущиеДанные)
	...
	ПараметрыОтображения = get_prop(ТекущийРаздел, "ПараметрыОтображения");
	Если ЗначениеЗаполнено(ПараметрыОтображения) И Не get_prop(ПараметрыОтображения, "COLUMNS") = Неопределено Тогда
		ПерваяКолонка = ТекущийРаздел.ПараметрыОтображения["COLUMNS"][0]["NAME"];
	Иначе
		ПерваяКолонка = "";
	КонецЕсли;
	Для каждого Потомок из Потомки Цикл
		Если get_prop(Потомок, ПерваяКолонка) = "Нет подчиненных" Или get_prop(Потомок, "ЗагрузитьЕще") = "Загрузить еще" Тогда
			Прервать;
		КонецЕсли;
...
&НаКлиенте
Процедура ОтметитьВсеИерархическийСписокУстановитьОтметку()
	Потомки = СтрокиЭлементаДереваНаКлиенте(ИерархическийСписок);
	Для Каждого Потомок Из Потомки Цикл
		Потомок.Отмечен = ЭтаФорма.ОтмеченыВсе;
		ПроставитьПометкиВниз(Потомок);
	КонецЦикла;
КонецПроцедуры
