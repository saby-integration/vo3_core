...
Процедура ЗаполнитьКолонкиИерархическогоСписка() Экспорт
	...
	Для Каждого Колонка Из ТекущийРаздел.ПараметрыОтображения["Columns"] Цикл
		...
		СвойстваКолонки = get_prop(Колонка, "Properties", Новый Массив); 
		Для Каждого СвойствоПоля Из СвойстваКолонки Цикл
...
Процедура ИерархическийСписокПередРазворачиванием(Элемент, Строка, Отказ)
	...
	Если Строки[0][ПерваяКолонка] = "Нет подчиненных" Тогда
		...
		Рез = ИерархическийСписокПередРазворачиваниемНаКлиенте(СтрокаСсылка);
...
&НаКлиенте
Функция ИерархическийСписокПередРазворачиваниемНаКлиенте(СтрокаСсылка)
	...
	Строка = ИерархическийСписок.Строки.НайтиСтроки(Новый Структура("Ссылка", СтрокаСсылка), Истина);
	Строка[0].Строки.Очистить();
	ЗаполнитьДанныеИерархическогоСписка(Строка[0], Данные);
	Возврат СтрокиДереваВМассив(Строка[0].Строки, ИерархическийСписок.Колонки);
КонецФункции
