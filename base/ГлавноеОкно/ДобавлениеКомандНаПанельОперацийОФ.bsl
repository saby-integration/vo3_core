...
Процедура ДобавитьКомандыНаПанельОпераций(ЭлемПанельОпераций, КомандыПанели, Лево) 
	...
	Если ИмяФормы = "ДиалогВыбораИзСписка" Тогда
		КомандаВыбрать = Новый Соответствие;
		КомандаВыбрать.Вставить("TEMPLATE", "command");
		КомандаВыбрать.Вставить("ACTION",	"ВыбратьНажатие");
		КомандаВыбрать.Вставить("SHOW_IN_OPERATION_PANEL",	Истина);
		КомандаВыбрать.Вставить("SHOW_IN_MAIN_TOOLBAR",	Истина);
		КомандаВыбрать.Вставить("DEFAULT_BUTTON",	Истина);
		КомандаВыбрать.Вставить("DESCRIPTION",	"Выбрать");
		КомандаВыбрать.Вставить("ARGS", Новый Структура);
		КомандаВыбрать.Вставить("SHOW_IN_ITEM_ACTION",	Истина);
		КомандаВыбрать.Вставить("TITLE", "Выбрать");
		КомандаВыбрать.Вставить("SHOW_IN_DEFAULT_TOOLBAR",	Ложь);
		КомандаВыбрать.Вставить("NAME", "КомандаВыбрать");
		КомандыПанели.Вставить(0, КомандаВыбрать);
	КонецЕсли;
...
Процедура ДобавитьПолеПоискаНаПанельОпераций(ЭлемПанельОпераций, Команда, Лево) 
	...
	Если ИмяФормы <> "ГлавноеОкно" И ИмяФормы <> "ДиалогВыбораИзСписка" Тогда
		Возврат;
	КонецЕсли;
	ИмяТаблицы = ЭтаФорма.ТекущийРаздел["Шаблон"];
	Элем = ЭлемФормы.Найти("ФильтрМаска" + ИмяТаблицы);
	Если Элем = Неопределено Тогда
		Элем = ЭлемФормы.Добавить(Тип("ПолеВвода"), "ФильтрМаска" + ИмяТаблицы, Истина, ЭлемПанельОпераций);
...
Процедура ДобавитьКнопкаСКомандойНаПанельОпераций(Команда, ЭлемПанельОпераций, Лево)
	Если Команда["TEMPLATE"] = "command" Тогда
		...
		Исключение 
			ЭтаФорма.ЭлементыФормы.Удалить(Элем);
			Возврат;
		КонецПопытки;
