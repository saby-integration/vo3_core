...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, НазваниеПродукта)
	...
	ИначеЕсли ИмяСобытия = НазваниеПродукта() + "SabySignOut" Тогда
...
Процедура ВыйтиНажатие(Элемент)
	...
	Оповестить(НазваниеПродукта() + "SabySignOut");
...
Функция ФильтрДляКоманды()
	...
	Selection.Вставить("MarkedAll", ЭтаФорма.ОтмеченыВсе);
	Selection.Вставить("Marked", СписокОтмеченных);
	МассивАлгоритмов = Новый Массив;
	МассивАлгоритмов.Добавить(ТекущийРаздел["Идентификатор"]);
	Selection.Вставить("DataSets ", МассивАлгоритмов);
	...
	КонецЦикла;
	Filter = КопияФильтраСНастроеннымПериодом(Filter);
	Возврат Filter;
КонецФункции
...
&НаКлиенте
Функция BLOCKLY_RUN(context_params, ПараметрыКоманды) Экспорт // Функция для автотестов BLOCKLY
	ЭтаФорма.context_params = context_params;
	ЗаписатьНастройкиНаСервере();
	...
	ТранспортИнтеграции.СброситьКэшНаСервере();
	Возврат BlocklyExecutor.API_BLOCKLY_RUN(ПараметрыКоманды);	
КонецФункции
