...
Процедура ДобавитьПолеПоискаНаПанельОпераций(ЭлемПанельОпераций, Команда) 
	...
	Если Элем = Неопределено Тогда
		...
		Если ЭлемРодитель <> Неопределено Тогда
			Элем = ДобавитьЭлементФормы("ФильтрМаска" + ИмяТаблицы, Тип("ПолеФормы"), ЭлемРодитель, Истина);
		Иначе
			Элем = ДобавитьЭлементФормы("ФильтрМаска" + ИмяТаблицы, Тип("ПолеФормы"), ЭлемПанельОпераций, Истина);
		КонецЕсли;
...
Процедура ДобавитьКнопкаСКомандойНаПанельОпераций(Команда, ЭлемПанельОпераций)
	Если Команда["TEMPLATE"] = "command" Тогда
		Элем = ЭлементКоманды(Команда, ЭлемПанельОпераций);
		Элем.Заголовок = Команда["TITLE"];
		Элем.Видимость = Истина;
...
Функция ЭлементКоманды(Команда, ЭлемПанельОпераций)
	ЭлемФормы = ПолучитьЭлементыФормыНаСервере();
	Элем = ЭлемФормы.Найти(Команда["NAME"]);
	Если Элем = Неопределено Тогда
	...
	Иначе
		ЭлемФормы.Переместить(Элем, ЭлемПанельОпераций);
	КонецЕсли;
	Возврат Элем;
КонецФункции
