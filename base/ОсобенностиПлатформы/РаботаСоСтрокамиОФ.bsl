...
//
//DynamicDirective
Функция Saby_СтрРазделить82(ИсходнаяСтрока, Разделитель, ВключатьПустые = Истина) Экспорт

	Подстроки			= Новый Массив;
	МногострочнаяСтрока	= ИсходнаяСтрока;
	Для Индекс = 1 По СтрДлина(Разделитель) Цикл
		
		МногострочнаяСтрока = СтрЗаменить(МногострочнаяСтрока, Сред(Разделитель, Индекс, 1), Символы.ПС);
		
	КонецЦикла;
	
	Для Индекс = 1 По СтрЧислоСтрок(МногострочнаяСтрока) Цикл
		
		ЗначениеСтроки = СтрПолучитьСтроку(МногострочнаяСтрока, Индекс);
		Если		ПустаяСтрока(ЗначениеСтроки)
			И	Не	ВключатьПустые Тогда
			
			Продолжить;
			
		КонецЕсли;
		Подстроки.Добавить(ЗначениеСтроки);
...
//
//DynamicDirective
Функция Saby_СтрСоединить82(Знач Строки, Разделитель = "") Экспорт
	Результат = "";
...
//DynamicDirective
Функция Saby_СтрШаблон82(Знач Шаблон, Параметр1 = Неопределено, Параметр2 = Неопределено, Параметр3 = Неопределено,
...
//
//DynamicDirective
Функция Saby_СтрНачинаетсяС82(Строка, СтрокаПоиска) Экспорт
...
//
//DynamicDirective
Функция Saby_СтрЗаканчиваетсяНа82(Строка, СтрокаПоиска) Экспорт
