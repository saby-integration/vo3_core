...
Процедура ЗагрузитьВложение(ДокументССылка, Файл, Название, context = Неопределено, block_context = Неопределено)
	...
	Если ВладелецФайлов.Метаданные().ПолноеИмя() = "Справочник.Сотрудники" Тогда
	...
	Если ТипЗнч(МодульЭДКО) = Тип("ОбщийМодуль") Тогда
		...
		//После ЗУП 3.1.22.77;
		МассивВсеВложений  = МодульЭДКО.ПрикрепленныеФайлыКОбъектуИзСправочника(ВладелецФайлов, ОбъектМетаданных.Имя);
