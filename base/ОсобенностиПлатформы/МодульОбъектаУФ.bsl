...
Функция ПолучитьФизЛицоСотрудника(Сотрудник)
	Результат = Сотрудник;
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ФизическоеЛицо", Сотрудник.Метаданные()) Тогда
...
Функция НайтиПрисоединенныйФайл(ДокументССылка, НазваниеБезРасширения, Тип)   
	...
	Для Каждого ПрисоединенныйФайл Из МассивВсеВложений Цикл
		...
		Если ПрисоединенныйФайл.Наименование = НазваниеБезРасширения Тогда
			...
			КонецЕсли;
			ВложениеНайдено = ПрисоединенныйФайлО;
			Прервать;
		КонецЕсли;
	КонецЦикла; 
	Возврат Неопределено;
...
Процедура ЗаписатьНовоеВложение(ВладелецФайлов, НазваниеБезРасширения, Тип, ПодписанЭП, Зашифрован, ВременноеХранилище) 
...
Процедура ЗаписатьСуществующееВложение(ПрисоединенныйФайл, ВременноеХранилище, ПодписанЭП, Зашифрован)
	ОбщийМодульРаботаСФайлами = ОбщийМодульКонфигурации("РаботаСФайлами");
	Если ОбщийМодульРаботаСФайлами = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьВложение(ДокументССылка, Файл, Название, context = Неопределено, block_context = Неопределено)
	...
	Для каждого ДанныеДокумента из массивДанныхСтатусаДокумента Цикл
		...
		Если ДокументСБИС = Неопределено и ЗначениеЗаполнено(ДанныеДокумента.UID) Тогда
			ДокументСБИС =	Транспорт.local_helper_read_document(context_params, Новый Структура("Идентификатор", ДанныеДокумента.UID));
	...
	Если Не ВложениеНайдено Тогда
		ЗаписатьНовоеВложение(ДокументССылка,НазваниеБезРасширения,Тип,ПодписанЭП,Зашифрован и ПодписанЭП,ВременноеХранилище);
	Иначе   
		ЗаписатьСуществующееВложение(ПрисоединенныйФайл, ВременноеХранилище, Зашифрован, Зашифрован и ПодписанЭП);
	КонецЕсли;
