...
Процедура ЗагрузитьВложение(ДокументССылка, Файл, Название, context = Неопределено, block_context = Неопределено)
	...
	Для каждого ДанныеДокумента из массивДанныхСтатусаДокумента Цикл
		...
		Для Каждого ФайлФложение Из ДокументВложения Цикл
			Если get_prop(ФайлФложение,"Название") <> Название Тогда
				Продолжить;
			КонецЕсли;
			МассивПодписейФайла = get_prop(ФайлФложение,"Подпись");
			ПодписанЭП = ТипЗнч(МассивПодписейФайла) = Тип("Массив") И МассивПодписейФайла.Количество() > 0;
			Зашифрован = Врег(get_prop(ФайлФложение,"Зашифрован")) = "ДА";
			Прервать;
		КонецЦикла;
	...
	Иначе
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Зашифрован", ПрисоединенныйФайл.Метаданные()) Тогда
			ПрисоединенныйФайлОбъект.Зашифрован = Зашифрован;
		КонецЕсли;
