...
Функция ПолучитьСписокПрикреплёныхФайлов(ВладелецФайлов)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СтатусыСинхронизацииФайловСОблачнымСервисом.УчетнаяЗапись, НЕОПРЕДЕЛЕНО) <> НЕОПРЕДЕЛЕНО
	|				И Файлы.Редактирует = НЕОПРЕДЕЛЕНО
	|			ТОГДА ЕСТЬNULL(СтатусыСинхронизацииФайловСОблачнымСервисом.УчетнаяЗапись, НЕОПРЕДЕЛЕНО)
...
Процедура ЗагрузитьВложение(ДокументССылка, Файл, Название, context = Неопределено, block_context = Неопределено)
	...
	Исключение
		массивДанныхСтатусаДокумента =  Новый массив;
